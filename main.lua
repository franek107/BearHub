...
local function PANIC_DESTROY()
	...
	MISC.InfAmmo = false
	MISC.NoClip = false
	MISC.SuperPunch = false
	MISC.RapidFire = false
	MISC.WalkSpeedEnabled = false
	MISC.JumpPowerEnabled = false
	...
	pcall(function()
		...
		if myChar then
			...
			if myHum then
				...
				-- Restore normal walk speed and jump power
				myHum.WalkSpeed = 16
	...
	-- Remove all hitbox highlights from all players
	pcall(function()
	...
	-- Remove all mesh modifications (restore original sizes)
	pcall(function()
		for _, plr in ipairs(Players:GetPlayers()) do
			if plr ~= player and plr.Character then
	...
	-- Clear all _G references
	pcall(function()
		_G.BearHub_playClick = nil
		_G.BearHub_playSlider = nil
		_G.BearHub_doClick = nil
		_G.BearHub_startDragSound = nil
		_G.BearHub_stopDragSound = nil
		_G.BearHub_startSpectate = nil
		_G.BearHub_stopSpectate = nil
		_G.BearHub_teleportTo = nil
		_G.BearHub_bringPlayer = nil
		_G.BearHub_switchPlaces = nil
		_G.BearHub_fullRefresh = nil
		_G.BearHub_getPos = nil
		_G.BearHub_visCheck = nil
		_G.BearHub_healPlayer = nil
		_G.BearHub_openCP = nil
		_G.BearHub_updCPValues = nil
		_G.BearHub_canvasDrag = nil
		_G.BearHub_hueDrag = nil
		_G.BearHub_allSliders = nil
		_G.BearHub_tabPages = nil
		_G.BearHub_refreshPlayerList = nil
	end)
	...
	-- Also search for any leftover GUIs
	pcall(function()
	...
	-- Stop any sounds we created
	pcall(function()
		for _, s in ipairs(SoundService:GetChildren()) do
			if s:IsA("Sound") and (s.SoundId == CLICK_SOUND_ID or s.SoundId == SLIDER_SOUND_ID or s.SoundId == DRAG_SOUND_ID) then
				s:Stop()
				s:Destroy()
			end
		end
...
do
	...
	local function updateESP()
		...
		for plr in pairs(inventoryCache) do
			if not cur[plr] then inventoryCache[plr] = nil; inventoryCacheTick[plr] = nil end
		end
							...
							if not skip then
								...
								if not skip then
									...
									if ESP.Distance.Enabled then
										...
										d.distance.TextColor3 = ESP.Distance.Color; d.distance.Visible = true
									...
									if ESP.Inventory.Enabled then
										...
										if #items > 0 then
											...
											if #invText > 40 then invText = string.sub(invText, 1, 37) .. "..." end
...
do
	...
	local function saveMeshData(part)
		...
		if mesh and not savedMeshScales[mesh] then
			...
			if mesh:IsA("SpecialMesh") then savedMeshOffsets[mesh] = mesh.Offset end
		end
	end
	...
	local function restoreMesh(part)
		...
		if mesh and savedMeshScales[mesh] then
			pcall(function()
				...
				if mesh:IsA("SpecialMesh") and savedMeshOffsets[mesh] then mesh.Offset = savedMeshOffsets[mesh] end
			end)
			savedMeshScales[mesh] = nil; savedMeshOffsets[mesh] = nil
		end
	end

	local function createHighlight(part)
		if hitboxHighlights[part] and hitboxHighlights[part].Parent then return hitboxHighlights[part] end
		...
		sphere.Name = hlName; sphere.Adornee = part
		sphere.Color3 = Color3.fromRGB(255, 60, 60)
		sphere.SurfaceColor3 = Color3.fromRGB(255, 60, 60)
		sphere.SurfaceTransparency = 0.7; sphere.Transparency = 0.5
		sphere.Parent = char
	...
	local function removeHighlight(part)
		if hitboxHighlights[part] then pcall(function() hitboxHighlights[part]:Destroy() end); hitboxHighlights[part] = nil end
	end
	...
	local function expandPart(part)
		...
		local mesh = findMesh(part)
		if mesh then saveMeshData(part); pcall(function() mesh.Scale = savedMeshScales[mesh] * scale end) end
		local hl = createHighlight(part)
		if hl then hl.Adornee = part; hl.Transparency = 0.5; hl.SurfaceTransparency = 0.7 end
	end

	local function restorePart(part)
		if not part or not part.Parent then return end
		restoreMesh(part); removeHighlight(part)
	end
	...
	local function restoreAllForChar(char)
		...
		for _, child in ipairs(char:GetChildren()) do
			if child.Name:find("BearHub_HL_") then pcall(function() child:Destroy() end) end
		end
	end
	...
	local function cleanupDead()
		...
		for _, mesh in ipairs(toRemoveMesh) do savedMeshScales[mesh] = nil; savedMeshOffsets[mesh] = nil end
		local toRemoveHL = {}
		for part in pairs(hitboxHighlights) do
			if not part or not part.Parent then table.insert(toRemoveHL, part) end
		end
		for _, part in ipairs(toRemoveHL) do hitboxHighlights[part] = nil end
	end
	...
	RunService.Heartbeat:Connect(function()
		...
		lastHitboxBone = HITBOX.Bone; lastHitboxEnabled = HITBOX.Enabled
		for _, plr in ipairs(Players:GetPlayers()) do
			if plr ~= player then
				local char = plr.Character
				if char then
					if HITBOX.Enabled and HITBOX.Size > 0 then
						...
						for _, part in ipairs(char:GetChildren()) do
							if part:IsA("BasePart") and hitboxHighlights[part] and not targetSet[part] then restorePart(part) end
						end
						for _, part in ipairs(targetParts) do pcall(function() expandPart(part) end) end
					else
	...
	Players.PlayerAdded:Connect(function(p)
		p.CharacterAdded:Connect(function() task.wait(1); cleanupDead() end)
		p.CharacterRemoving:Connect(function() local char = p.Character; if char then restoreAllForChar(char) end end)
	end)
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= player then
			p.CharacterAdded:Connect(function() task.wait(1); cleanupDead() end)
...
do
	...
	player.CharacterAdded:Connect(function(c) task.wait(0.5); hookHumanoid(c) end)
	...
	task.spawn(function()
		while true do
			...
			if char then
				local hum = char:FindFirstChildOfClass("Humanoid")
				if hum then
					if MISC.WalkSpeedEnabled then pcall(function() hum.WalkSpeed = MISC.WalkSpeed end) end
					if MISC.JumpPowerEnabled then pcall(function() hum.UseJumpPower = true; hum.JumpPower = MISC.JumpPower end) end
				end
			end
		end
	end)
	...
	local function startFly()
		...
		task.spawn(function()
			while flying and MISC.NoClip and not PANIC_TRIGGERED do
				...
				local moveVec = (camCFrame.LookVector * forward + camCFrame.RightVector * right + Vector3.new(0, up, 0))
				if moveVec.Magnitude > 0 then moveVec = moveVec.Unit * speed end
				pcall(function()
					if flyBodyVelocity and flyBodyVelocity.Parent then flyBodyVelocity.Velocity = moveVec end
					if flyBodyGyro and flyBodyGyro.Parent then flyBodyGyro.CFrame = camCFrame end
				end)
			end
			stopFly()
		end)
					...
					pcall(function()
						for _, v in ipairs(tool:GetDescendants()) do
							if v:IsA("NumberValue") or v:IsA("IntValue") then
								...
								if MISC.NoRecoil then
									if n:find("recoil") or n:find("kick") or n:find("camerashake") or n:find("shake") then v.Value = 0 end
								end
								if MISC.NoSpread then
									if n:find("spread") or n:find("bulletspread") or n:find("firerandom") then v.Value = 0 end
									if n:find("accuracy") then v.Value = 1 end
								end
								if MISC.InfAmmo then
									if n == "ammo" or n == "currentammo" or n == "bullets" or n == "storedammo" or n:find("magazine") or n:find("clip") or n:find("mag") or n == "maxammo" or n == "reserveammo" or n == "totalammo" then v.Value = 999 end
								end
								if MISC.RapidFire then
									...
									if n:find("firerate") or n:find("fire_rate") or n:find("rateof") then
										if rfMultiplier > 0 then if v.Value < 9000 then v.Value = v.Value / math.max(rfMultiplier, 0.01) end
										else v.Value = 99999 end
									end
									if n:find("firedelay") or n:find("fire_delay") or n:find("delay") or n:find("cooldown") or n:find("cool_down") or n:find("firecooldown") or n:find("shotcooldown") or n:find("shotdelay") or n:find("shot_delay") or n:find("fireinterval") or n:find("interval") or n:find("timebetween") or n:find("attackspeed") or n:find("attackcooldown") or n:find("attackdelay") or n:find("shootdelay") or n:find("shootcooldown") then
										v.Value = v.Value * rfMultiplier
									end
									if n:find("automatic") or n:find("auto") or n:find("isautomatic") then if v.Value == 0 then v.Value = 1 end end
								end
							end
							if v:IsA("BoolValue") then
								local n = v.Name:lower()
								if MISC.InfAmmo then if n:find("reloading") or n:find("isreloading") then v.Value = false end end
								if MISC.RapidFire then
						...
						for name, val in pairs(attrs) do
							...
							if type(val) == "number" then
								if MISC.NoRecoil and (n:find("recoil") or n:find("kick")) then tool:SetAttribute(name, 0) end
								if MISC.NoSpread and (n:find("spread") or n:find("accuracy")) then tool:SetAttribute(name, 0) end
								if MISC.InfAmmo and (n == "ammo" or n:find("magazine") or n:find("clip")) then tool:SetAttribute(name, 999) end
								if MISC.RapidFire then
									local rfLevel = MISC.RapidFireLevel or 20; local rfMultiplier = rfLevel / 20
									if n:find("firedelay") or n:find("cooldown") or n:find("firerate") or n:find("delay") or n:find("interval") or n:find("shotdelay") then
										if n:find("rate") then if rfMultiplier > 0 then tool:SetAttribute(name, val / math.max(rfMultiplier, 0.01)) else tool:SetAttribute(name, 99999) end
										else tool:SetAttribute(name, val * rfMultiplier) end
									end
								end
							end
							if type(val) == "boolean" and MISC.RapidFire then
								if n:find("canfire") or n:find("canshoot") or n:find("ready") then tool:SetAttribute(name, true) end
							end
						end
					end)
				end
	...
	task.spawn(function() while true do task.wait(3); if PANIC_TRIGGERED then break end; if player.Character then pcall(function() scanTools(player.Character) end) end; pcall(scanBackpack) end end)
			...
			pcall(function()
				for _, v in ipairs(parent:GetChildren()) do
					if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") then
						local n = v.Name:lower()
						if n:find("damage") or n:find("hit") or n:find("punch") or n:find("attack") or n:find("melee") or n:find("fight") or n:find("combat") or n:find("strike") or n:find("swing") or n:find("weapon") or n:find("fist") or n:find("pi[eę]s?[cć]") or n:find("dmg") or n:find("kill") or n:find("hurt") then
							table.insert(remotes, v)
						end
					end
					if v:IsA("Folder") or v:IsA("Model") or v:IsA("Configuration") then scan(v, depth + 1) end
				end
			end)
		end
		scan(ReplicatedStorage, 0); scan(workspace, 0)
		return remotes
	end
	...
	local function hookPunchTool(tool)
		...
		local activated = tool.Activated:Connect(function()
			if not MISC.SuperPunch or PANIC_TRIGGERED then return end
					...
					if plr ~= player and plr.Character then
						...
						if hum and hum.Health > 0 and targetRoot then
							...
							if dist <= bestDist then
								...
								if dot > 0.2 then best = plr; bestDist = dist end
							end
						end
					end
				end
			...
			task.spawn(function()
				for i = 1, multiplier do
					if not MISC.SuperPunch or PANIC_TRIGGERED then break end
					...
					for _, remote in ipairs(remotes) do
						pcall(function()
							if remote:IsA("RemoteEvent") then
								remote:FireServer(target); remote:FireServer(target, targetHum)
								remote:FireServer(targetHum); remote:FireServer(targetChar)
		...
		tool.AncestryChanged:Connect(function()
			if not tool.Parent then punchedTools[tool] = nil; if activated then activated:Disconnect() end end
		end)
	end

	local function scanForPunchTools()
		local char = player.Character
		if char then for _, t in ipairs(char:GetChildren()) do if t:IsA("Tool") then hookPunchTool(t) end end end
		local bp = player:FindFirstChildOfClass("Backpack")
		if bp then for _, t in ipairs(bp:GetChildren()) do if t:IsA("Tool") then hookPunchTool(t) end end end
	end
	scanForPunchTools()
	if player.Character then player.Character.ChildAdded:Connect(function(c) if c:IsA("Tool") then hookPunchTool(c) end end) end
	player.CharacterAdded:Connect(function(c) task.wait(0.5); c.ChildAdded:Connect(function(ch) if ch:IsA("Tool") then hookPunchTool(ch) end end); scanForPunchTools() end)
	local bp = player:FindFirstChildOfClass("Backpack")
	if bp then bp.ChildAdded:Connect(function(c) if c:IsA("Tool") then hookPunchTool(c) end end) end
	task.spawn(function() while true do task.wait(2); if PANIC_TRIGGERED then break end; pcall(scanForPunchTools) end end)
	...
	local function isMouseOverGui()
		...
		for _, obj in ipairs(guiObjects) do
			local current = obj
			while current do
				if current == gui or current.Name == "BearHub" then return true end
...
do
	...
	cpClose.Size = UDim2.new(0, 25, 0, 25); cpClose.Position = UDim2.new(1, -30, 0, 3)
	cpClose.BackgroundTransparency = 1; cpClose.Text = "X"
	cpClose.TextColor3 = Color3.fromRGB(200,200,200); cpClose.Font = Enum.Font.GothamBold
	cpClose.TextSize = 16; cpClose.ZIndex = 102
	...
	satOver.Size = UDim2.new(1,0,1,0); satOver.BackgroundColor3 = Color3.new(1,1,1)
	satOver.BorderSizePixel = 0; satOver.ZIndex = 102
	...
	valOver.Size = UDim2.new(1,0,1,0); valOver.BackgroundColor3 = Color3.new(0,0,0)
	valOver.BorderSizePixel = 0; valOver.ZIndex = 103
	...
	hueSlider.BackgroundColor3 = Color3.new(1,1,1); hueSlider.BorderSizePixel = 0; hueSlider.ZIndex = 102
	...
	cpPrev.BackgroundColor3 = Color3.fromRGB(255,0,0); cpPrev.BorderSizePixel = 0; cpPrev.ZIndex = 101
	...
	cpGB.Size = UDim2.new(1,0,1,0); cpGB.BackgroundTransparency = 1; cpGB.Text = ""; cpGB.ZIndex = 106
	...
	hB.InputBegan:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			cH = math.clamp((i.Position.X - ap.X) / as.X, 0, 1); updCP()
		end
	end)
	...
	_G.BearHub_updCPValues = function(x, y, kind)
		if kind == "canvas" then
			local ap = cpGrid.AbsolutePosition; local as = cpGrid.AbsoluteSize
			cS = math.clamp((x - ap.X) / as.X, 0, 1); cV = 1 - math.clamp((y - ap.Y) / as.Y, 0, 1); updCP()
		elseif kind == "hue" then
			local ap = hueBar.AbsolutePosition; local as = hueBar.AbsoluteSize
			cH = math.clamp((x - ap.X) / as.X, 0, 1); updCP()
		end
	end
end
...
do
	...
	mkSection = function(parent, text, order)
		...
		l.Size = UDim2.new(1, 0, 0, 28); l.BackgroundTransparency = 1; l.Text = text
		l.TextColor3 = Color3.fromRGB(160,160,170); l.Font = Enum.Font.GothamBold
		l.TextSize = 14; l.TextXAlignment = Enum.TextXAlignment.Left; l.LayoutOrder = order or 0
	end

	mkCheck = function(parent, text, tbl, key, order)
		local h = Instance.new("Frame", parent)
		h.Size = UDim2.new(1,0,0,30); h.BackgroundTransparency = 1; h.LayoutOrder = order or 0
		local en = tbl[key] or false
		local box = Instance.new("TextButton", h)
		box.Size = UDim2.new(0,22,0,22); box.Position = UDim2.new(0,5,0.5,-11)
		...
		check.Size = UDim2.new(0.75,0,0.75,0); check.Position = UDim2.new(0.125,0,0.125,0)
		check.BackgroundTransparency = 1; check.Image = CHECK_ICON; check.ImageColor3 = Color3.new(1,1,1); check.Visible = en
		local lbl = Instance.new("TextLabel", h)
		lbl.Size = UDim2.new(1,-40,1,0); lbl.Position = UDim2.new(0,35,0,0); lbl.BackgroundTransparency = 1
		lbl.Text = text; lbl.TextColor3 = Color3.fromRGB(200,200,210); lbl.Font = Enum.Font.Gotham
		lbl.TextSize = 13; lbl.TextXAlignment = Enum.TextXAlignment.Left
		box.MouseButton1Click:Connect(function()
			playClick(); en = not en; box.BackgroundColor3 = en and PURPLE or GRAY; check.Visible = en; tbl[key] = en
	...
	mkCheckColor = function(parent, text, tbl, key, colorKey, order)
		...
		check.BackgroundTransparency = 1; check.Image = CHECK_ICON; check.ImageColor3 = Color3.new(1,1,1); check.Visible = en
		local lbl = Instance.new("TextLabel", h)
		lbl.Size = UDim2.new(1,-80,1,0); lbl.Position = UDim2.new(0,35,0,0); lbl.BackgroundTransparency = 1
		lbl.Text = text; lbl.TextColor3 = Color3.fromRGB(200,200,210); lbl.Font = Enum.Font.Gotham
		lbl.TextSize = 13; lbl.TextXAlignment = Enum.TextXAlignment.Left
		local circle = Instance.new("TextButton", h)
		circle.Size = UDim2.new(0,22,0,22); circle.Position = UDim2.new(1,-30,0.5,-11)
		circle.BackgroundColor3 = colorRef; circle.Text = ""; circle.AutoButtonColor = false; circle.BorderSizePixel = 0
		Instance.new("UICorner", circle).CornerRadius = UDim.new(1,0)
		Instance.new("UIStroke", circle).Color = Color3.fromRGB(80,80,90)
		circle.MouseButton1Click:Connect(function()
			playClick(); openCP(circle.BackgroundColor3, function(nc)
				circle.BackgroundColor3 = nc
				if isSubTable then ESP[key].Color = nc else tbl[colorKey] = nc end
			end)
		end)
		box.MouseButton1Click:Connect(function()
			playClick(); en = not en; box.BackgroundColor3 = en and PURPLE or GRAY; check.Visible = en
			if isSubTable then ESP[key].Enabled = en else tbl[key] = en end
		end)
	end

	mkSlider = function(parent, text, minV, maxV, def, suf, tbl, key, order)
		local h = Instance.new("Frame", parent)
		h.Size = UDim2.new(1,0,0,50); h.BackgroundTransparency = 1; h.LayoutOrder = order or 0
		local val = def or minV
		local lbl = Instance.new("TextLabel", h)
		lbl.Size = UDim2.new(0.6,0,0,20); lbl.Position = UDim2.new(0,5,0,0); lbl.BackgroundTransparency = 1
		lbl.Text = text; lbl.TextColor3 = Color3.fromRGB(200,200,210); lbl.Font = Enum.Font.GothamBold
		lbl.TextSize = 13; lbl.TextXAlignment = Enum.TextXAlignment.Left
		local vl = Instance.new("TextLabel", h)
		vl.Size = UDim2.new(0.4,-5,0,20); vl.Position = UDim2.new(0.6,0,0,0); vl.BackgroundTransparency = 1
		vl.Text = tostring(val)..(suf or ""); vl.TextColor3 = Color3.fromRGB(150,150,160)
		vl.Font = Enum.Font.Gotham; vl.TextSize = 13; vl.TextXAlignment = Enum.TextXAlignment.Right
		local bg = Instance.new("Frame", h)
		bg.Size = UDim2.new(1,-10,0,6); bg.Position = UDim2.new(0,5,0,30)
		bg.BackgroundColor3 = Color3.fromRGB(50,50,60); bg.BorderSizePixel = 0
		Instance.new("UICorner", bg).CornerRadius = UDim.new(1,0)
		local pct = (val - minV) / (maxV - minV)
		local fill = Instance.new("Frame", bg)
		fill.Size = UDim2.new(pct,0,1,0); fill.BackgroundColor3 = PURPLE; fill.BorderSizePixel = 0
		...
		hit.Size = UDim2.new(1,20,0,30); hit.Position = UDim2.new(0,-10,0.5,-15)
		hit.BackgroundTransparency = 1; hit.Text = ""; hit.ZIndex = 3
		local drag = false; local lastSliderSoundTick = 0; local lastSliderVal = val
		local function upd(x)
			...
			vl.Text = tostring(val)..(suf or ""); if tbl and key then tbl[key] = val end
			if val ~= lastSliderVal then
				local now = tick()
				if now - lastSliderSoundTick > 0.08 then lastSliderSoundTick = now; playSlider() end
				lastSliderVal = val
			end
		end
	...
	mkDropdown = function(parent, text, opts, tbl, key, order)
		local h = Instance.new("Frame", parent)
		h.Size = UDim2.new(1,0,0,60); h.BackgroundTransparency = 1; h.LayoutOrder = order or 0; h.ClipsDescendants = false
		local lbl = Instance.new("TextLabel", h)
		lbl.Size = UDim2.new(1,0,0,20); lbl.BackgroundTransparency = 1; lbl.Text = text
		lbl.TextColor3 = Color3.fromRGB(200,200,210); lbl.Font = Enum.Font.GothamBold; lbl.TextSize = 13
		lbl.TextXAlignment = Enum.TextXAlignment.Left; lbl.Position = UDim2.new(0,5,0,0)
		local btn = Instance.new("TextButton", h)
		btn.Size = UDim2.new(1,-10,0,30); btn.Position = UDim2.new(0,5,0,25)
		btn.BackgroundColor3 = Color3.fromRGB(40,40,50); btn.BorderSizePixel = 0
		btn.Text = "  "..tbl[key]; btn.TextColor3 = Color3.fromRGB(200,200,210)
		btn.Font = Enum.Font.Gotham; btn.TextSize = 13; btn.TextXAlignment = Enum.TextXAlignment.Left; btn.AutoButtonColor = false
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0,6)
		local dd = Instance.new("Frame", h)
		dd.Size = UDim2.new(1,-10,0,#opts*30); dd.Position = UDim2.new(0,5,0,58)
		...
		Instance.new("UIListLayout", dd).Padding = UDim.new(0,0)
		for _, opt in ipairs(opts) do
			local ob = Instance.new("TextButton", dd)
			ob.Size = UDim2.new(1,0,0,30); ob.BackgroundColor3 = Color3.fromRGB(35,35,45)
			ob.Text = "  "..opt; ob.TextColor3 = Color3.fromRGB(180,180,190)
			ob.Font = Enum.Font.Gotham; ob.TextSize = 13; ob.TextXAlignment = Enum.TextXAlignment.Left
			ob.AutoButtonColor = false; ob.ZIndex = 51; ob.BorderSizePixel = 0
			...
			ob.MouseButton1Click:Connect(function() playClick(); tbl[key] = opt; btn.Text = "  "..opt; dd.Visible = false end)
		end
	...
	for _, kc in ipairs(KB_KEYS) do
		local kcc = kc; table.insert(BIND_OPTIONS, {kc.Name, function() return UIS:IsKeyDown(kcc) end})
	end
	...
	mkKeybind = function(parent, text, tbl, order)
		...
		check.BackgroundTransparency = 1; check.Image = CHECK_ICON; check.ImageColor3 = Color3.new(1,1,1); check.Visible = en
		local lbl = Instance.new("TextLabel", h)
		lbl.Size = UDim2.new(0,55,1,0); lbl.Position = UDim2.new(0,35,0,0); lbl.BackgroundTransparency = 1
		lbl.Text = text; lbl.TextColor3 = Color3.fromRGB(200,200,210); lbl.Font = Enum.Font.Gotham
		lbl.TextSize = 13; lbl.TextXAlignment = Enum.TextXAlignment.Left
		local keyBtn = Instance.new("TextButton", h)
		keyBtn.Size = UDim2.new(0,110,0,24); keyBtn.Position = UDim2.new(1,-115,0.5,-12)
		keyBtn.BackgroundColor3 = Color3.fromRGB(40,40,50); keyBtn.BorderSizePixel = 0
		keyBtn.Text = tbl.KeybindName; keyBtn.TextColor3 = Color3.fromRGB(180,180,190)
		keyBtn.Font = Enum.Font.GothamBold; keyBtn.TextSize = 11; keyBtn.AutoButtonColor = false
		Instance.new("UICorner", keyBtn).CornerRadius = UDim.new(0,5)
		local totalOpts = #BIND_OPTIONS + 1
		local ddFrame = Instance.new("Frame", h)
		ddFrame.Size = UDim2.new(0,170,0,math.min(totalOpts,8)*28); ddFrame.Position = UDim2.new(1,-175,1,2)
		ddFrame.BackgroundColor3 = Color3.fromRGB(30,30,38); ddFrame.BorderSizePixel = 0
		ddFrame.Visible = false; ddFrame.ZIndex = 200; ddFrame.ClipsDescendants = true
		Instance.new("UICorner", ddFrame).CornerRadius = UDim.new(0,6)
		local dds = Instance.new("UIStroke", ddFrame); dds.Color = PURPLE; dds.Thickness = 1.5
		local ddScroll = Instance.new("ScrollingFrame", ddFrame)
		ddScroll.Size = UDim2.new(1,0,1,0); ddScroll.BackgroundTransparency = 1
		ddScroll.ScrollBarThickness = 3; ddScroll.ScrollBarImageColor3 = PURPLE
		ddScroll.CanvasSize = UDim2.new(0,0,0,totalOpts*28); ddScroll.ZIndex = 201
		Instance.new("UIListLayout", ddScroll).Padding = UDim.new(0,0)
		local noneBtn = Instance.new("TextButton", ddScroll)
		noneBtn.Size = UDim2.new(1,0,0,28); noneBtn.BackgroundColor3 = Color3.fromRGB(30,30,38)
		noneBtn.Text = "  NONE"; noneBtn.TextColor3 = Color3.fromRGB(150,150,160)
		noneBtn.Font = Enum.Font.Gotham; noneBtn.TextSize = 12; noneBtn.TextXAlignment = Enum.TextXAlignment.Left
		noneBtn.AutoButtonColor = false; noneBtn.ZIndex = 202; noneBtn.BorderSizePixel = 0; noneBtn.LayoutOrder = 0
		...
		noneBtn.MouseButton1Click:Connect(function()
			playClick(); tbl.KeybindName = "NONE"; tbl.KeybindCheck = nil; keyBtn.Text = "NONE"; ddFrame.Visible = false
		end)
		for i, opt in ipairs(BIND_OPTIONS) do
			local name = opt[1]; local checkFn = opt[2]
			local ob = Instance.new("TextButton", ddScroll)
			ob.Size = UDim2.new(1,0,0,28); ob.BackgroundColor3 = Color3.fromRGB(30,30,38)
			ob.Text = "  "..name; ob.TextColor3 = Color3.fromRGB(180,180,190)
			ob.Font = Enum.Font.Gotham; ob.TextSize = 12; ob.TextXAlignment = Enum.TextXAlignment.Left
			ob.AutoButtonColor = false; ob.ZIndex = 202; ob.BorderSizePixel = 0; ob.LayoutOrder = i
			ob.MouseEnter:Connect(function() ob.BackgroundColor3 = Color3.fromRGB(50,50,65) end)
			ob.MouseLeave:Connect(function() ob.BackgroundColor3 = Color3.fromRGB(30,30,38) end)
			ob.MouseButton1Click:Connect(function()
				playClick(); tbl.KeybindName = name; tbl.KeybindCheck = checkFn; keyBtn.Text = name; ddFrame.Visible = false
			end)
		end
		local ddOpen = false
		keyBtn.MouseButton1Click:Connect(function() playClick(); ddOpen = not ddOpen; ddFrame.Visible = ddOpen end)
		box.MouseButton1Click:Connect(function()
			playClick(); en = not en; tbl.Enabled = en; box.BackgroundColor3 = en and PURPLE or GRAY; check.Visible = en
		end)
	end

	mkButton = function(parent, text, callback, order, customColor)
		local btn = Instance.new("TextButton", parent)
		btn.Size = UDim2.new(1,-10,0,36); btn.BackgroundColor3 = customColor or PURPLE; btn.BorderSizePixel = 0
		btn.Text = text; btn.TextColor3 = Color3.new(1,1,1); btn.Font = Enum.Font.GothamBold
		btn.TextSize = 13; btn.AutoButtonColor = false; btn.LayoutOrder = order or 0
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0,6)
		local baseColor = customColor or PURPLE
		...
		btn.MouseButton1Click:Connect(function()
			playClick(); if callback then pcall(callback) end
		end)
		return btn
	end
	...
	createPage = function(name)
		...
		p.ScrollBarThickness = 3; p.ScrollBarImageColor3 = PURPLE
...
do
	...
	vL.Size = UDim2.new(0.48,0,0,260); vL.Position = UDim2.new(0,10,0,5)
	vL.BackgroundColor3 = DARK; vL.BorderSizePixel = 0
	Instance.new("UICorner", vL).CornerRadius = UDim.new(0,8)
	local vll = Instance.new("UIListLayout", vL); vll.Padding = UDim.new(0,4); vll.SortOrder = Enum.SortOrder.LayoutOrder
	local vlp = Instance.new("UIPadding", vL); vlp.PaddingTop = UDim.new(0,8); vlp.PaddingLeft = UDim.new(0,5); vlp.PaddingRight = UDim.new(0,5)

	local vR = Instance.new("Frame", vizPage)
	vR.Size = UDim2.new(0.48,0,0,360); vR.Position = UDim2.new(0.5,5,0,5)
	vR.BackgroundColor3 = DARK; vR.BorderSizePixel = 0
	Instance.new("UICorner", vR).CornerRadius = UDim.new(0,8)
	local vrl = Instance.new("UIListLayout", vR); vrl.Padding = UDim.new(0,4); vrl.SortOrder = Enum.SortOrder.LayoutOrder
	local vrp = Instance.new("UIPadding", vR); vrp.PaddingTop = UDim.new(0,8); vrp.PaddingLeft = UDim.new(0,5); vrp.PaddingRight = UDim.new(0,5)
	...
	subBar.Size = UDim2.new(1,-20,0,30); subBar.Position = UDim2.new(0,10,0,0); subBar.BackgroundTransparency = 1
	...
	subPagesFrame.Size = UDim2.new(1,0,1,-40); subPagesFrame.Position = UDim2.new(0,0,0,38); subPagesFrame.BackgroundTransparency = 1

	local tbPage = Instance.new("Frame", subPagesFrame); tbPage.Size = UDim2.new(1,0,1,0); tbPage.BackgroundTransparency = 1; tbPage.Visible = true
	local tbL = Instance.new("Frame", tbPage); tbL.Size = UDim2.new(0.48,0,0,300); tbL.Position = UDim2.new(0,10,0,5); tbL.BackgroundColor3 = DARK; tbL.BorderSizePixel = 0
	Instance.new("UICorner", tbL).CornerRadius = UDim.new(0,8)
	local tbll = Instance.new("UIListLayout", tbL); tbll.Padding = UDim.new(0,4); tbll.SortOrder = Enum.SortOrder.LayoutOrder
	local tblp = Instance.new("UIPadding", tbL); tblp.PaddingTop = UDim.new(0,8); tblp.PaddingLeft = UDim.new(0,5); tblp.PaddingRight = UDim.new(0,5)
	local tbR = Instance.new("Frame", tbPage); tbR.Size = UDim2.new(0.48,0,0,300); tbR.Position = UDim2.new(0.5,5,0,5); tbR.BackgroundColor3 = DARK; tbR.BorderSizePixel = 0
	Instance.new("UICorner", tbR).CornerRadius = UDim.new(0,8)
	local tbrl = Instance.new("UIListLayout", tbR); tbrl.Padding = UDim.new(0,4); tbrl.SortOrder = Enum.SortOrder.LayoutOrder
	local tbrp = Instance.new("UIPadding", tbR); tbrp.PaddingTop = UDim.new(0,8); tbrp.PaddingLeft = UDim.new(0,5); tbrp.PaddingRight = UDim.new(0,5)
	...
	local abPage = Instance.new("Frame", subPagesFrame); abPage.Size = UDim2.new(1,0,1,0); abPage.BackgroundTransparency = 1; abPage.Visible = false
	local abL = Instance.new("Frame", abPage); abL.Size = UDim2.new(0.48,0,0,300); abL.Position = UDim2.new(0,10,0,5); abL.BackgroundColor3 = DARK; abL.BorderSizePixel = 0
	Instance.new("UICorner", abL).CornerRadius = UDim.new(0,8)
	local abll = Instance.new("UIListLayout", abL); abll.Padding = UDim.new(0,4); abll.SortOrder = Enum.SortOrder.LayoutOrder
	local ablp = Instance.new("UIPadding", abL); ablp.PaddingTop = UDim.new(0,8); ablp.PaddingLeft = UDim.new(0,5); ablp.PaddingRight = UDim.new(0,5)
	local abR = Instance.new("Frame", abPage); abR.Size = UDim2.new(0.48,0,0,360); abR.Position = UDim2.new(0.5,5,0,5); abR.BackgroundColor3 = DARK; abR.BorderSizePixel = 0
	Instance.new("UICorner", abR).CornerRadius = UDim.new(0,8)
	local abrl = Instance.new("UIListLayout", abR); abrl.Padding = UDim.new(0,4); abrl.SortOrder = Enum.SortOrder.LayoutOrder
	local abrp = Instance.new("UIPadding", abR); abrp.PaddingTop = UDim.new(0,8); abrp.PaddingLeft = UDim.new(0,5); abrp.PaddingRight = UDim.new(0,5)

	mkSection(abL, "Aimbot", 1); mkKeybind(abL, "Enable", AIMBOT, 2)
	mkCheckColor(abL, "Draw FOV", AIMBOT, "DrawFOV", "FOVColor", 3)
	mkCheck(abL, "Visible Check", AIMBOT, "VisibleCheck", 4); mkCheck(abL, "Exclude Dead", AIMBOT, "ExcludeDead", 5)
	...
	local hbPage = Instance.new("Frame", subPagesFrame); hbPage.Size = UDim2.new(1,0,1,0); hbPage.BackgroundTransparency = 1; hbPage.Visible = false
	local hbL = Instance.new("Frame", hbPage); hbL.Size = UDim2.new(0.48,0,0,250); hbL.Position = UDim2.new(0,10,0,5); hbL.BackgroundColor3 = DARK; hbL.BorderSizePixel = 0
	Instance.new("UICorner", hbL).CornerRadius = UDim.new(0,8)
	local hbll = Instance.new("UIListLayout", hbL); hbll.Padding = UDim.new(0,4); hbll.SortOrder = Enum.SortOrder.LayoutOrder
	local hblp = Instance.new("UIPadding", hbL); hblp.PaddingTop = UDim.new(0,8); hblp.PaddingLeft = UDim.new(0,5); hblp.PaddingRight = UDim.new(0,5)
	...
	local function switchSub(name)
		tbPage.Visible = (name == "TriggerBot"); abPage.Visible = (name == "Aimbot"); hbPage.Visible = (name == "Hitbox")
	end
	local function mkSB(name, order)
		...
		btn.Text = name; btn.TextColor3 = Color3.fromRGB(120,120,130); btn.Font = Enum.Font.GothamBold
		btn.TextSize = 14; btn.AutoButtonColor = false; btn.LayoutOrder = order
		local ul = Instance.new("Frame", btn)
		ul.Size = UDim2.new(1,0,0,2); ul.Position = UDim2.new(0,0,1,-2)
		ul.BackgroundColor3 = PURPLE; ul.BorderSizePixel = 0; ul.Visible = false
		btn.MouseButton1Click:Connect(function()
			playClick()
			if selSub then selSub.btn.TextColor3 = Color3.fromRGB(120,120,130); selSub.ul.Visible = false end
			selSub = {btn = btn, ul = ul}; btn.TextColor3 = Color3.new(1,1,1); ul.Visible = true; switchSub(name)
		end)
		return {btn = btn, ul = ul}
	end
	local s1 = mkSB("TriggerBot", 1); mkSB("Aimbot", 2); mkSB("Hitbox", 3)
	selSub = s1; s1.btn.TextColor3 = Color3.new(1,1,1); s1.ul.Visible = true
end

--============================================================
-- BLOK 9: MISC PAGE + AUTOFARM PAGE + PLAYERS PAGE + SETTINGS (with PANIC)
--============================================================
do
	-- MISC PAGE
	local miscPage = createPage("Miscellaneous")
	local mL = Instance.new("Frame", miscPage)
	mL.Size = UDim2.new(0.48,0,0,700); mL.Position = UDim2.new(0,10,0,5); mL.BackgroundColor3 = DARK; mL.BorderSizePixel = 0
	Instance.new("UICorner", mL).CornerRadius = UDim.new(0,8)
	local mll = Instance.new("UIListLayout", mL); mll.Padding = UDim.new(0,4); mll.SortOrder = Enum.SortOrder.LayoutOrder
	local mlp = Instance.new("UIPadding", mL); mlp.PaddingTop = UDim.new(0,8); mlp.PaddingLeft = UDim.new(0,5); mlp.PaddingRight = UDim.new(0,5)
	...
	mR.Size = UDim2.new(0.48,0,0,150); mR.Position = UDim2.new(0.5,5,0,5); mR.BackgroundColor3 = DARK; mR.BorderSizePixel = 0
	Instance.new("UICorner", mR).CornerRadius = UDim.new(0,8)
	local mrl = Instance.new("UIListLayout", mR); mrl.Padding = UDim.new(0,6); mrl.SortOrder = Enum.SortOrder.LayoutOrder
	local mrp = Instance.new("UIPadding", mR); mrp.PaddingTop = UDim.new(0,8); mrp.PaddingLeft = UDim.new(0,5); mrp.PaddingRight = UDim.new(0,5)
	mkSection(mR, "Options", 1)
	mkButton(mR, "Heal", healPlayer, 2)

	-- AUTOFARM PAGE
	local autofarmPage = createPage("AutoFarm")
	local afLbl = Instance.new("TextLabel", autofarmPage)
	afLbl.Size = UDim2.new(1,-20,0,40); afLbl.Position = UDim2.new(0,10,0,10)
	afLbl.BackgroundTransparency = 1; afLbl.Text = "AutoFarm - Coming Soon"
	afLbl.TextColor3 = Color3.fromRGB(100,100,110); afLbl.Font = Enum.Font.Gotham; afLbl.TextSize = 16

	-- PLAYERS PAGE
	local playersPage = createPage("Players")
	local plListFrame = Instance.new("Frame", playersPage)
	plListFrame.Size = UDim2.new(0.42,0,1,-10); plListFrame.Position = UDim2.new(0,10,0,5)
	plListFrame.BackgroundColor3 = DARK; plListFrame.BorderSizePixel = 0
	Instance.new("UICorner", plListFrame).CornerRadius = UDim.new(0,8)

	local plListTitle = Instance.new("TextLabel", plListFrame)
	plListTitle.Size = UDim2.new(1,-100,0,25); plListTitle.Position = UDim2.new(0,10,0,5)
	plListTitle.BackgroundTransparency = 1; plListTitle.Text = "Players in Server"
	plListTitle.TextColor3 = Color3.fromRGB(160,160,170); plListTitle.Font = Enum.Font.GothamBold
	plListTitle.TextSize = 14; plListTitle.TextXAlignment = Enum.TextXAlignment.Left

	local plCountLbl = Instance.new("TextLabel", plListFrame)
	plCountLbl.Size = UDim2.new(0,85,0,25); plCountLbl.Position = UDim2.new(1,-90,0,5)
	plCountLbl.BackgroundTransparency = 1; plCountLbl.Text = "0 players"
	plCountLbl.TextColor3 = Color3.fromRGB(150,150,160); plCountLbl.Font = Enum.Font.Gotham
	plCountLbl.TextSize = 12; plCountLbl.TextXAlignment = Enum.TextXAlignment.Right

	local plScroll = Instance.new("ScrollingFrame", plListFrame)
	plScroll.Size = UDim2.new(1,-10,1,-40); plScroll.Position = UDim2.new(0,5,0,32)
	plScroll.BackgroundTransparency = 1; plScroll.ScrollBarThickness = 3; plScroll.ScrollBarImageColor3 = PURPLE
	plScroll.CanvasSize = UDim2.new(0,0,0,0); plScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; plScroll.BorderSizePixel = 0
	Instance.new("UIListLayout", plScroll).Padding = UDim.new(0,4)

	local plInfoFrame = Instance.new("Frame", playersPage)
	plInfoFrame.Size = UDim2.new(0.55,0,1,-10); plInfoFrame.Position = UDim2.new(0.44,5,0,5)
	plInfoFrame.BackgroundColor3 = DARK; plInfoFrame.BorderSizePixel = 0
	Instance.new("UICorner", plInfoFrame).CornerRadius = UDim.new(0,8)

	local plInfoTitle = Instance.new("TextLabel", plInfoFrame)
	plInfoTitle.Size = UDim2.new(1,-20,0,22); plInfoTitle.Position = UDim2.new(0,10,0,8)
	plInfoTitle.BackgroundTransparency = 1; plInfoTitle.Text = "Selected Player"
	plInfoTitle.TextColor3 = Color3.fromRGB(160,160,170); plInfoTitle.Font = Enum.Font.GothamBold
	plInfoTitle.TextSize = 14; plInfoTitle.TextXAlignment = Enum.TextXAlignment.Left

	local plAvatarFrame = Instance.new("Frame", plInfoFrame)
	plAvatarFrame.Size = UDim2.new(0,60,0,60); plAvatarFrame.Position = UDim2.new(0.5,-30,0,35)
	plAvatarFrame.BackgroundColor3 = Color3.fromRGB(50,50,60); plAvatarFrame.BorderSizePixel = 0
	Instance.new("UICorner", plAvatarFrame).CornerRadius = UDim.new(1,0)
	local plAvatar = Instance.new("ImageLabel", plAvatarFrame)
	plAvatar.Size = UDim2.new(1,0,1,0); plAvatar.BackgroundTransparency = 1; plAvatar.Image = ""; plAvatar.ScaleType = Enum.ScaleType.Crop
	Instance.new("UICorner", plAvatar).CornerRadius = UDim.new(1,0)

	local plNameLbl = Instance.new("TextLabel", plInfoFrame)
	plNameLbl.Size = UDim2.new(1,-20,0,18); plNameLbl.Position = UDim2.new(0,10,0,100); plNameLbl.BackgroundTransparency = 1
	plNameLbl.Text = "No player selected"; plNameLbl.TextColor3 = Color3.new(1,1,1)
	plNameLbl.Font = Enum.Font.GothamBold; plNameLbl.TextSize = 14; plNameLbl.TextXAlignment = Enum.TextXAlignment.Center

	local plUsernameLbl = Instance.new("TextLabel", plInfoFrame)
	plUsernameLbl.Size = UDim2.new(1,-20,0,15); plUsernameLbl.Position = UDim2.new(0,10,0,119); plUsernameLbl.BackgroundTransparency = 1
	plUsernameLbl.Text = ""; plUsernameLbl.TextColor3 = Color3.fromRGB(150,150,160)
	plUsernameLbl.Font = Enum.Font.Gotham; plUsernameLbl.TextSize = 11; plUsernameLbl.TextXAlignment = Enum.TextXAlignment.Center

	local plIdLbl = Instance.new("TextLabel", plInfoFrame)
	plIdLbl.Size = UDim2.new(0.5,-15,0,15); plIdLbl.Position = UDim2.new(0,10,0,136); plIdLbl.BackgroundTransparency = 1
	plIdLbl.Text = ""; plIdLbl.TextColor3 = Color3.fromRGB(150,150,160)
	plIdLbl.Font = Enum.Font.Gotham; plIdLbl.TextSize = 11; plIdLbl.TextXAlignment = Enum.TextXAlignment.Center

	local plDistLbl = Instance.new("TextLabel", plInfoFrame)
	plDistLbl.Size = UDim2.new(0.5,-15,0,15); plDistLbl.Position = UDim2.new(0.5,5,0,136); plDistLbl.BackgroundTransparency = 1
	plDistLbl.Text = ""; plDistLbl.TextColor3 = Color3.fromRGB(100,200,255)
	plDistLbl.Font = Enum.Font.GothamBold; plDistLbl.TextSize = 11; plDistLbl.TextXAlignment = Enum.TextXAlignment.Center

	local row1 = Instance.new("Frame", plInfoFrame)
	row1.Size = UDim2.new(1,-20,0,28); row1.Position = UDim2.new(0,10,0,160); row1.BackgroundTransparency = 1
	local plSpectateBtn = Instance.new("TextButton", row1)
	plSpectateBtn.Size = UDim2.new(0.5,-3,1,0); plSpectateBtn.BackgroundColor3 = PURPLE; plSpectateBtn.BorderSizePixel = 0
	plSpectateBtn.Text = "Spectate"; plSpectateBtn.TextColor3 = Color3.new(1,1,1); plSpectateBtn.Font = Enum.Font.GothamBold
	plSpectateBtn.TextSize = 13; plSpectateBtn.AutoButtonColor = false
	Instance.new("UICorner", plSpectateBtn).CornerRadius = UDim.new(0,6)
	local plUnspectateBtn = Instance.new("TextButton", row1)
	plUnspectateBtn.Size = UDim2.new(0.5,-3,1,0); plUnspectateBtn.Position = UDim2.new(0.5,3,0,0)
	plUnspectateBtn.BackgroundColor3 = Color3.fromRGB(180,60,60); plUnspectateBtn.BorderSizePixel = 0
	plUnspectateBtn.Text = "Unspectate"; plUnspectateBtn.TextColor3 = Color3.new(1,1,1)
	plUnspectateBtn.Font = Enum.Font.GothamBold; plUnspectateBtn.TextSize = 13; plUnspectateBtn.AutoButtonColor = false
	Instance.new("UICorner", plUnspectateBtn).CornerRadius = UDim.new(0,6)

	local row2 = Instance.new("Frame", plInfoFrame)
	row2.Size = UDim2.new(1,-20,0,28); row2.Position = UDim2.new(0,10,0,193); row2.BackgroundTransparency = 1
	local plTeleportBtn = Instance.new("TextButton", row2)
	plTeleportBtn.Size = UDim2.new(0.5,-3,1,0); plTeleportBtn.BackgroundColor3 = Color3.fromRGB(60,140,220)
	plTeleportBtn.BorderSizePixel = 0; plTeleportBtn.Text = "Teleport"; plTeleportBtn.TextColor3 = Color3.new(1,1,1)
	plTeleportBtn.Font = Enum.Font.GothamBold; plTeleportBtn.TextSize = 13; plTeleportBtn.AutoButtonColor = false
	Instance.new("UICorner", plTeleportBtn).CornerRadius = UDim.new(0,6)
	local plBringBtn = Instance.new("TextButton", row2)
	plBringBtn.Size = UDim2.new(0.5,-3,1,0); plBringBtn.Position = UDim2.new(0.5,3,0,0)
	plBringBtn.BackgroundColor3 = Color3.fromRGB(80,180,100); plBringBtn.BorderSizePixel = 0
	plBringBtn.Text = "Bring"; plBringBtn.TextColor3 = Color3.new(1,1,1)
	plBringBtn.Font = Enum.Font.GothamBold; plBringBtn.TextSize = 13; plBringBtn.AutoButtonColor = false
	Instance.new("UICorner", plBringBtn).CornerRadius = UDim.new(0,6)

	local plSwitchBtn = Instance.new("TextButton", plInfoFrame)
	plSwitchBtn.Size = UDim2.new(1,-20,0,28); plSwitchBtn.Position = UDim2.new(0,10,0,226)
	plSwitchBtn.BackgroundColor3 = Color3.fromRGB(220,150,50); plSwitchBtn.BorderSizePixel = 0
	plSwitchBtn.Text = "Switch Places"; plSwitchBtn.TextColor3 = Color3.new(1,1,1)
	plSwitchBtn.Font = Enum.Font.GothamBold; plSwitchBtn.TextSize = 13; plSwitchBtn.AutoButtonColor = false
	Instance.new("UICorner", plSwitchBtn).CornerRadius = UDim.new(0,6)

	local plStatusLbl = Instance.new("TextLabel", plInfoFrame)
	plStatusLbl.Size = UDim2.new(1,-20,0,20); plStatusLbl.Position = UDim2.new(0,10,1,-28)
	plStatusLbl.BackgroundTransparency = 1; plStatusLbl.Text = ""
	plStatusLbl.TextColor3 = Color3.fromRGB(100,200,100); plStatusLbl.Font = Enum.Font.GothamBold
	plStatusLbl.TextSize = 12; plStatusLbl.TextXAlignment = Enum.TextXAlignment.Center

	local selectedPlayer = nil
	local playerButtons = {}

	local function showStatus(text, color, duration)
		plStatusLbl.Text = text; plStatusLbl.TextColor3 = color or Color3.fromRGB(100,200,100)
		if duration then task.spawn(function() task.wait(duration); if plStatusLbl.Text == text then plStatusLbl.Text = "" end end) end
	end
	...
	local function updateSelectedPlayerInfo()
		if selectedPlayer and selectedPlayer.Parent then
			...
			plUsernameLbl.Text = "@"..selectedPlayer.Name; plIdLbl.Text = "ID: "..selectedPlayer.UserId
			local dist = getDistanceToPlayer(selectedPlayer)
			plDistLbl.Text = dist and ("Distance: "..dist.."m") or "Distance: N/A"
			pcall(function() plAvatar.Image = Players:GetUserThumbnailAsync(selectedPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size150x150) end)
			if SPECTATE.Active and SPECTATE.Target == selectedPlayer then plStatusLbl.Text = "SPECTATING"; plStatusLbl.TextColor3 = Color3.fromRGB(100,200,100) end
		else
			plNameLbl.Text = "No player selected"; plUsernameLbl.Text = ""; plIdLbl.Text = ""; plDistLbl.Text = ""; plAvatar.Image = ""; plStatusLbl.Text = ""
		end
	end
	task.spawn(function() while true do task.wait(0.5); if PANIC_TRIGGERED then break end; if selectedPlayer and selectedPlayer.Parent then local d = getDistanceToPlayer(selectedPlayer); plDistLbl.Text = d and ("Distance: "..d.."m") or "Distance: N/A" end end end)
	local function selectPlayer(plr)
		selectedPlayer = plr
		for p, data in pairs(playerButtons) do data.btn.BackgroundColor3 = (p == plr) and Color3.fromRGB(70,50,140) or Color3.fromRGB(40,40,50) end
		updateSelectedPlayerInfo()
	end
	local function createPlayerButton(plr)
		if playerButtons[plr] then return end
		local btn = Instance.new("TextButton", plScroll)
		btn.Size = UDim2.new(1,-6,0,42); btn.BackgroundColor3 = Color3.fromRGB(40,40,50); btn.BorderSizePixel = 0
		btn.Text = ""; btn.AutoButtonColor = false; btn.LayoutOrder = plr.UserId
		Instance.new("UICorner", btn).CornerRadius = UDim.new(0,6)
		local avatarFrame = Instance.new("Frame", btn)
		avatarFrame.Size = UDim2.new(0,32,0,32); avatarFrame.Position = UDim2.new(0,5,0.5,-16)
		avatarFrame.BackgroundColor3 = Color3.fromRGB(50,50,60); avatarFrame.BorderSizePixel = 0
		Instance.new("UICorner", avatarFrame).CornerRadius = UDim.new(1,0)
		local avatar = Instance.new("ImageLabel", avatarFrame)
		avatar.Size = UDim2.new(1,0,1,0); avatar.BackgroundTransparency = 1; avatar.Image = ""; avatar.ScaleType = Enum.ScaleType.Crop
		Instance.new("UICorner", avatar).CornerRadius = UDim.new(1,0)
		task.spawn(function() pcall(function() avatar.Image = Players:GetUserThumbnailAsync(plr.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48) end) end)
		local nameLbl = Instance.new("TextLabel", btn)
		nameLbl.Size = UDim2.new(1,-45,0,18); nameLbl.Position = UDim2.new(0,42,0,3); nameLbl.BackgroundTransparency = 1
		nameLbl.Text = plr.DisplayName or plr.Name; nameLbl.TextColor3 = Color3.new(1,1,1)
		nameLbl.Font = Enum.Font.GothamBold; nameLbl.TextSize = 13; nameLbl.TextXAlignment = Enum.TextXAlignment.Left; nameLbl.TextTruncate = Enum.TextTruncate.AtEnd
		local userLbl = Instance.new("TextLabel", btn)
		userLbl.Size = UDim2.new(1,-45,0,16); userLbl.Position = UDim2.new(0,42,0,21); userLbl.BackgroundTransparency = 1
		userLbl.Text = "@"..plr.Name; userLbl.TextColor3 = Color3.fromRGB(150,150,160)
		userLbl.Font = Enum.Font.Gotham; userLbl.TextSize = 11; userLbl.TextXAlignment = Enum.TextXAlignment.Left; userLbl.TextTruncate = Enum.TextTruncate.AtEnd
		btn.MouseEnter:Connect(function() if selectedPlayer ~= plr then btn.BackgroundColor3 = Color3.fromRGB(50,50,65) end end)
		btn.MouseLeave:Connect(function() if selectedPlayer ~= plr then btn.BackgroundColor3 = Color3.fromRGB(40,40,50) end end)
		btn.MouseButton1Click:Connect(function() playClick(); selectPlayer(plr) end)
		playerButtons[plr] = {btn = btn}
	end
	local function removePlayerButton(plr)
		if playerButtons[plr] then pcall(function() playerButtons[plr].btn:Destroy() end); playerButtons[plr] = nil end
		if selectedPlayer == plr then selectedPlayer = nil; updateSelectedPlayerInfo() end
	end
	local function refreshPlayerList()
		local toRemove = {}
		for plr in pairs(playerButtons) do if not plr or not plr.Parent then table.insert(toRemove, plr) end end
		for _, plr in ipairs(toRemove) do removePlayerButton(plr) end
		local count = 0
		for _, plr in ipairs(Players:GetPlayers()) do
			if plr ~= player then if not playerButtons[plr] then createPlayerButton(plr) end; count = count + 1 end
		end
		plCountLbl.Text = count.." player"..(count == 1 and "" or "s")
	end
	_G.BearHub_refreshPlayerList = refreshPlayerList
	Players.PlayerAdded:Connect(function(plr) task.wait(0.5); if plr ~= player and plr.Parent and not PANIC_TRIGGERED then pcall(refreshPlayerList) end end)
	Players.PlayerRemoving:Connect(function(plr) removePlayerButton(plr); task.wait(0.1); pcall(refreshPlayerList) end)
	task.spawn(function() task.wait(1); pcall(refreshPlayerList) end)
	task.spawn(function() while true do task.wait(3); if PANIC_TRIGGERED then break end; pcall(refreshPlayerList) end end)
	...
	plSpectateBtn.MouseButton1Click:Connect(function()
		playClick()
		if selectedPlayer and selectedPlayer.Parent then startSpectate(selectedPlayer); showStatus("SPECTATING", Color3.fromRGB(100,200,100))
		else showStatus("Select a player first!", Color3.fromRGB(255,100,100), 2) end
	end)
	...
	plUnspectateBtn.MouseButton1Click:Connect(function() playClick(); stopSpectate(); showStatus("Stopped spectating", Color3.fromRGB(150,150,160), 1.5) end)
	...
	plTeleportBtn.MouseButton1Click:Connect(function()
		playClick()
		if selectedPlayer and selectedPlayer.Parent then local ok,msg = teleportTo(selectedPlayer); showStatus(msg, ok and Color3.fromRGB(100,200,255) or Color3.fromRGB(255,100,100), 2)
		else showStatus("Select a player first!", Color3.fromRGB(255,100,100), 2) end
	end)
	...
	plBringBtn.MouseButton1Click:Connect(function()
		playClick()
		if selectedPlayer and selectedPlayer.Parent then local ok,msg = bringPlayer(selectedPlayer); showStatus(msg, ok and Color3.fromRGB(120,220,130) or Color3.fromRGB(255,100,100), 2)
		else showStatus("Select a player first!", Color3.fromRGB(255,100,100), 2) end
	end)
	...
	plSwitchBtn.MouseButton1Click:Connect(function()
		playClick()
		if selectedPlayer and selectedPlayer.Parent then local ok,msg = switchPlaces(selectedPlayer); showStatus(msg, ok and Color3.fromRGB(255,180,80) or Color3.fromRGB(255,100,100), 2)
		else showStatus("Select a player first!", Color3.fromRGB(255,100,100), 2) end
	end)

	-- ============================================
	-- SETTINGS PAGE WITH PANIC SUB-TAB
	-- ============================================
	local settingsPage = createPage("Settings")

	local settingsSubBar = Instance.new("Frame", settingsPage)
	settingsSubBar.Size = UDim2.new(1,-20,0,30); settingsSubBar.Position = UDim2.new(0,10,0,0); settingsSubBar.BackgroundTransparency = 1
	local settingsSbl = Instance.new("UIListLayout", settingsSubBar); settingsSbl.FillDirection = Enum.FillDirection.Horizontal; settingsSbl.Padding = UDim.new(0,15)

	local settingsSubPages = Instance.new("Frame", settingsPage)
	settingsSubPages.Size = UDim2.new(1,0,1,-40); settingsSubPages.Position = UDim2.new(0,0,0,38); settingsSubPages.BackgroundTransparency = 1

	-- General settings sub-page
	local settingsGeneralPage = Instance.new("Frame", settingsSubPages)
	settingsGeneralPage.Size = UDim2.new(1,0,1,0); settingsGeneralPage.BackgroundTransparency = 1; settingsGeneralPage.Visible = true
	local sgLbl = Instance.new("TextLabel", settingsGeneralPage)
	sgLbl.Size = UDim2.new(1,-20,0,40); sgLbl.Position = UDim2.new(0,10,0,10); sgLbl.BackgroundTransparency = 1
	sgLbl.Text = "General Settings - Coming Soon"; sgLbl.TextColor3 = Color3.fromRGB(100,100,110)
	sgLbl.Font = Enum.Font.Gotham; sgLbl.TextSize = 16

	-- Panic sub-page
	local settingsPanicPage = Instance.new("Frame", settingsSubPages)
	settingsPanicPage.Size = UDim2.new(1,0,1,0); settingsPanicPage.BackgroundTransparency = 1; settingsPanicPage.Visible = false

	-- Panic content
	local panicContainer = Instance.new("Frame", settingsPanicPage)
	panicContainer.Size = UDim2.new(0.7,0,0,300); panicContainer.Position = UDim2.new(0.15,0,0,20)
	panicContainer.BackgroundColor3 = DARK; panicContainer.BorderSizePixel = 0
	Instance.new("UICorner", panicContainer).CornerRadius = UDim.new(0,10)

	local panicIcon = Instance.new("TextLabel", panicContainer)
	panicIcon.Size = UDim2.new(1,0,0,50); panicIcon.Position = UDim2.new(0,0,0,20)
	panicIcon.BackgroundTransparency = 1; panicIcon.Text = "⚠️"
	panicIcon.TextColor3 = Color3.fromRGB(255,60,60); panicIcon.Font = Enum.Font.GothamBold; panicIcon.TextSize = 40

	local panicTitle = Instance.new("TextLabel", panicContainer)
	panicTitle.Size = UDim2.new(1,-20,0,25); panicTitle.Position = UDim2.new(0,10,0,75)
	panicTitle.BackgroundTransparency = 1; panicTitle.Text = "PANIC BUTTON"
	panicTitle.TextColor3 = Color3.fromRGB(255,80,80); panicTitle.Font = Enum.Font.GothamBold; panicTitle.TextSize = 20

	local panicDesc = Instance.new("TextLabel", panicContainer)
	panicDesc.Size = UDim2.new(1,-30,0,50); panicDesc.Position = UDim2.new(0,15,0,105)
	panicDesc.BackgroundTransparency = 1; panicDesc.TextWrapped = true
	panicDesc.Text = "Instantly destroys the entire script, removes all traces (ESP, hitbox modifications, GUI elements, sounds). It will look like you never used any cheats."
	panicDesc.TextColor3 = Color3.fromRGB(160,160,170); panicDesc.Font = Enum.Font.Gotham; panicDesc.TextSize = 12

	local panicBtn = Instance.new("TextButton", panicContainer)
	panicBtn.Size = UDim2.new(0.6,0,0,60); panicBtn.Position = UDim2.new(0.2,0,0,175)
	panicBtn.BackgroundColor3 = Color3.fromRGB(200,30,30); panicBtn.BorderSizePixel = 0
	panicBtn.Text = "SPIERDALAJ"; panicBtn.TextColor3 = Color3.new(1,1,1)
	panicBtn.Font = Enum.Font.GothamBold; panicBtn.TextSize = 24; panicBtn.AutoButtonColor = false
	Instance.new("UICorner", panicBtn).CornerRadius = UDim.new(0,10)
	local panicStroke = Instance.new("UIStroke", panicBtn)
	panicStroke.Color = Color3.fromRGB(255,60,60); panicStroke.Thickness = 2

	panicBtn.MouseEnter:Connect(function()
		panicBtn.BackgroundColor3 = Color3.fromRGB(255,40,40)
		panicStroke.Color = Color3.fromRGB(255,100,100)
	end)
	panicBtn.MouseLeave:Connect(function()
		panicBtn.BackgroundColor3 = Color3.fromRGB(200,30,30)
		panicStroke.Color = Color3.fromRGB(255,60,60)
	end)
	panicBtn.MouseButton1Click:Connect(function()
		PANIC_DESTROY()
	end)

	local panicHint = Instance.new("TextLabel", panicContainer)
	panicHint.Size = UDim2.new(1,-20,0,20); panicHint.Position = UDim2.new(0,10,0,245)
	panicHint.BackgroundTransparency = 1
	panicHint.Text = "Hotkey: Press END to panic instantly"
	panicHint.TextColor3 = Color3.fromRGB(100,100,110); panicHint.Font = Enum.Font.Gotham; panicHint.TextSize = 11

	-- Settings sub-tab buttons
	local selSettingsSub = nil
	local function switchSettingsSub(name)
		settingsGeneralPage.Visible = (name == "General")
		settingsPanicPage.Visible = (name == "Panic")
	end
	local function mkSettingsSB(name, order)
		local btn = Instance.new("TextButton", settingsSubBar)
		btn.Size = UDim2.new(0,100,1,0); btn.BackgroundTransparency = 1; btn.BorderSizePixel = 0
		btn.Text = name; btn.TextColor3 = Color3.fromRGB(120,120,130); btn.Font = Enum.Font.GothamBold
		btn.TextSize = 14; btn.AutoButtonColor = false; btn.LayoutOrder = order
		local ul = Instance.new("Frame", btn)
		...
		btn.MouseButton1Click:Connect(function()
			playClick()
			if selSettingsSub then selSettingsSub.btn.TextColor3 = Color3.fromRGB(120,120,130); selSettingsSub.ul.Visible = false end
			selSettingsSub = {btn = btn, ul = ul}; btn.TextColor3 = Color3.new(1,1,1); ul.Visible = true; switchSettingsSub(name)
		end)
		return {btn = btn, ul = ul}
	end
	local ss1 = mkSettingsSB("General", 1)
	mkSettingsSB("Panic", 2)
	selSettingsSub = ss1; ss1.btn.TextColor3 = Color3.new(1,1,1); ss1.ul.Visible = true
end

--============================================================
-- BLOK 10: TABS + MINIMIZE/DRAG + PANIC HOTKEY
--============================================================
local tabsFrame = sidebar:FindFirstChild("TabsFrame")
local tabsData = {{"AimAssistance"},{"Visualization"},{"Miscellaneous"},{"AutoFarm"},{"Players"},{"Settings"}}
local selTab = nil
...
miniBall.Size = UDim2.new(0,0,0,0); miniBall.Position = UDim2.new(0,40,0.5,-30)
...
local function minimize()
	...
	local t = TweenService:Create(main, TW, {Size = UDim2.new(0,0,0,0), Position = UDim2.new(0,cx,0,cy), BackgroundTransparency = 1})
	t:Play()
	miniBall.Position = UDim2.new(0,cx-30,0,cy-30); miniBall.Size = UDim2.new(0,0,0,0); miniBall.Visible = true
	TweenService:Create(miniBall, TW, {Size = BALL_SIZE}):Play()
	t.Completed:Connect(function() main.Visible = false; main.BackgroundTransparency = 0; animating = false end)
end

local function restore()
	if animating or not minimized then return end
	animating = true; minimized = false; playClick()
	local ap = miniBall.AbsolutePosition; local as = miniBall.AbsoluteSize
	local cx = ap.X + as.X/2; local cy = ap.Y + as.Y/2
	main.Size = UDim2.new(0,0,0,0); main.Position = UDim2.new(0,cx,0,cy)
	main.BackgroundTransparency = 1; main.Visible = true
	TweenService:Create(main, TW, {Size = ORIGINAL_SIZE, Position = UDim2.new(0,cx-350,0,cy-225), BackgroundTransparency = 0}):Play()
	local t2 = TweenService:Create(miniBall, TW, {Size = UDim2.new(0,0,0,0)})
	t2:Play()
	t2.Completed:Connect(function() miniBall.Visible = false; miniBall.Size = BALL_SIZE; animating = false end)
end
...
UIS.InputBegan:Connect(function(inp, gp)
	...
	-- PANIC HOTKEY: END key
	if inp.KeyCode == Enum.KeyCode.End then
		PANIC_DESTROY()
	end
end)

local dragging = false; local dragStart = nil; local startPos = nil; local mainDragMoved = false
...
local ballDrag = false; local ballStart = nil; local ballPos = nil; local ballMoved = false; local lastClickTime = 0
...
UIS.InputChanged:Connect(function(inp)
	...
	if inp.UserInputType == Enum.UserInputType.MouseMovement then
		if dragging and dragStart and startPos then
			...
			if d.Magnitude > 3 then if not mainDragMoved then mainDragMoved = true; startDragSound() end end
			main.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + d.X, startPos.Y.Scale, startPos.Y.Offset + d.Y)
		end
		if ballDrag and ballStart and ballPos then
			local d = inp.Position - ballStart
			if d.Magnitude > 3 then if not ballMoved then ballMoved = true; startDragSound() end end
...
UIS.InputEnded:Connect(function(inp)
	if inp.UserInputType == Enum.UserInputType.MouseButton1 then
		...
		if not PANIC_TRIGGERED then
			...
			if _G.BearHub_allSliders then for _, s in ipairs(_G.BearHub_allSliders) do s.setDrag(false) end end
		end
		if ballDrag then
