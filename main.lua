...
do
    ...
    pcall(function()
        ...
        if info and info.Name then
            ...
            if n:find("south bronx") or n:find("trencher") then
                IS_SOUTH_BRONX = true
            end
        end
    end)
    if SOUTH_BRONX_IDS[gameId] then
        IS_SOUTH_BRONX = true
    end
    pcall(function()
        if workspace:FindFirstChild("SouthBronx") or workspace:FindFirstChild("Trenchers") or workspace:FindFirstChild("TrenchMap") then
            IS_SOUTH_BRONX = true
        end
        for _, v in ipairs(workspace:GetChildren()) do
            if v.Name:lower():find("bronx") or v.Name:lower():find("trench") then
                IS_SOUTH_BRONX = true
                break
            end
        end
...
local function southBronxKick(featureName)
    if IS_SOUTH_BRONX then
        pcall(function()
            player:Kick("Anti-Cheat: " .. featureName .. " detected. You have been removed.")
        end)
...
local function serializeCurrentConfig()
    return {
        ESP = {
            ...
            ShowLocalPlayer = ESP.ShowLocalPlayer,
            VisibleOnly = ESP.VisibleOnly,
            Box = {Enabled = ESP.Box.Enabled, Color = colorToTable(ESP.Box.Color)},
            Skeleton = {Enabled = ESP.Skeleton.Enabled, Color = colorToTable(ESP.Skeleton.Color)},
            Name = {Enabled = ESP.Name.Enabled, Color = colorToTable(ESP.Name.Color)},
            ID = {Enabled = ESP.ID.Enabled, Color = colorToTable(ESP.ID.Color)},
            HealthBar = {Enabled = ESP.HealthBar.Enabled, Color = colorToTable(ESP.HealthBar.Color)},
            Distance = {Enabled = ESP.Distance.Enabled, Color = colorToTable(ESP.Distance.Color)},
            Snaplines = {Enabled = ESP.Snaplines.Enabled, Color = colorToTable(ESP.Snaplines.Color)},
            Inventory = {Enabled = ESP.Inventory.Enabled, Color = colorToTable(ESP.Inventory.Color)},
        },
        TRIGGERBOT = {
            Enabled = TRIGGERBOT.Enabled,
            KeybindName = TRIGGERBOT.KeybindName,
            Type = TRIGGERBOT.Type,
            ShowFOV = TRIGGERBOT.ShowFOV,
            FOVColor = colorToTable(TRIGGERBOT.FOVColor),
            FOV = TRIGGERBOT.FOV,
            ExcludeDead = TRIGGERBOT.ExcludeDead,
            VisibleOnly = TRIGGERBOT.VisibleOnly,
            MaxDistance = TRIGGERBOT.MaxDistance,
            ShotDelay = TRIGGERBOT.ShotDelay,
        },
        AIMBOT = {
            Enabled = AIMBOT.Enabled,
            KeybindName = AIMBOT.KeybindName,
            DrawFOV = AIMBOT.DrawFOV,
            FOVColor = colorToTable(AIMBOT.FOVColor),
            VisibleCheck = AIMBOT.VisibleCheck,
            ExcludeDead = AIMBOT.ExcludeDead,
            Bone = AIMBOT.Bone,
            FOV = AIMBOT.FOV,
            MaxDistance = AIMBOT.MaxDistance,
            SmoothX = AIMBOT.SmoothX,
            SmoothY = AIMBOT.SmoothY,
        },
        HITBOX = {
        ...
        MISC = {
            SemiGod = MISC.SemiGod,
            NoRecoil = MISC.NoRecoil,
            NoSpread = MISC.NoSpread,
            InfAmmo = MISC.InfAmmo,
            SuperPunch = MISC.SuperPunch,
            NoClip = MISC.NoClip,
            NoClipSpeed = MISC.NoClipSpeed,
            RapidFire = MISC.RapidFire,
            RapidFireLevel = MISC.RapidFireLevel,
            WalkSpeedEnabled = MISC.WalkSpeedEnabled,
            WalkSpeed = MISC.WalkSpeed,
            JumpPowerEnabled = MISC.JumpPowerEnabled,
            JumpPower = MISC.JumpPower,
            FreeCam = MISC.FreeCam,
            FreeCamSpeed = MISC.FreeCamSpeed,
        }
    }
end
...
if IS_SOUTH_BRONX then
    ...
    warnLbl2.Size = UDim2.new(1, 0, 0, 30)
    warnLbl2.BackgroundTransparency = 1
    warnLbl2.Text = "⚠️ South Bronx detected: FreeCam will KICK you!"
    warnLbl2.TextColor3 = Color3.fromRGB(255, 80, 80)
    warnLbl2.Font = Enum.Font.GothamBold
    warnLbl2.TextSize = 12
    warnLbl2.TextWrapped = true
    warnLbl2.LayoutOrder = 4
end
...
if IS_SOUTH_BRONX then
    ...
    sbInfo.Size = UDim2.new(1, 0, 0, 25)
    sbInfo.BackgroundTransparency = 1
    sbInfo.Text = "🎯 South Bronx detected - NoClip/FreeCam = KICK"
    sbInfo.TextColor3 = Color3.fromRGB(255, 120, 60)
    sbInfo.Font = Enum.Font.GothamBold
    sbInfo.TextSize = 11
    sbInfo.TextWrapped = true
    sbInfo.LayoutOrder = 3
end

--============================================================
-- CONFIG SUB-PAGE (SETTINGS)
--============================================================
local stConfigP = Instance.new("Frame", stSubPF)
stConfigP.Size = UDim2.new(1, 0, 1, 0)
stConfigP.BackgroundTransparency = 1
stConfigP.Visible = false

-- LEFT SIDE: Configs List
local cfgListFrame = Instance.new("Frame", stConfigP)
cfgListFrame.Size = UDim2.new(0.42, 0, 1, -10)
cfgListFrame.Position = UDim2.new(0, 10, 0, 5)
cfgListFrame.BackgroundColor3 = DARK
...
cfgListTitle.Size = UDim2.new(1, -10, 0, 25)
cfgListTitle.Position = UDim2.new(0, 10, 0, 8)
cfgListTitle.BackgroundTransparency = 1
cfgListTitle.Text = "Saved Configs"
cfgListTitle.TextColor3 = Color3.fromRGB(160, 160, 170)
cfgListTitle.Font = Enum.Font.GothamBold
cfgListTitle.TextSize = 14
cfgListTitle.TextXAlignment = Enum.TextXAlignment.Left

local cfgCountLabel = Instance.new("TextLabel", cfgListFrame)
cfgCountLabel.Size = UDim2.new(0, 80, 0, 25)
cfgCountLabel.Position = UDim2.new(1, -85, 0, 8)
cfgCountLabel.BackgroundTransparency = 1
cfgCountLabel.Text = "0 configs"
cfgCountLabel.TextColor3 = Color3.fromRGB(120, 120, 130)
cfgCountLabel.Font = Enum.Font.Gotham
cfgCountLabel.TextSize = 11
cfgCountLabel.TextXAlignment = Enum.TextXAlignment.Right

local cfgScroll = Instance.new("ScrollingFrame", cfgListFrame)
cfgScroll.Size = UDim2.new(1, -10, 1, -42)
cfgScroll.Position = UDim2.new(0, 5, 0, 38)
cfgScroll.BackgroundTransparency = 1
cfgScroll.ScrollBarThickness = 3
cfgScroll.ScrollBarImageColor3 = PURPLE
...
-- RIGHT SIDE: Config Panel Actions
local cfgActionsFrame = Instance.new("Frame", stConfigP)
cfgActionsFrame.Size = UDim2.new(0.55, 0, 1, -10)
cfgActionsFrame.Position = UDim2.new(0.44, 5, 0, 5)
cfgActionsFrame.BackgroundColor3 = DARK
cfgActionsFrame.BorderSizePixel = 0
Instance.new("UICorner", cfgActionsFrame).CornerRadius = UDim.new(0, 8)

local cfgActTitle = Instance.new("TextLabel", cfgActionsFrame)
cfgActTitle.Size = UDim2.new(1, -20, 0, 25)
cfgActTitle.Position = UDim2.new(0, 10, 0, 8)
cfgActTitle.BackgroundTransparency = 1
cfgActTitle.Text = "Config Actions"
cfgActTitle.TextColor3 = Color3.fromRGB(160, 160, 170)
cfgActTitle.Font = Enum.Font.GothamBold
...
cfgNameLabel.Position = UDim2.new(0, 10, 0, 40)
cfgNameLabel.BackgroundTransparency = 1
cfgNameLabel.Text = "Config Name"
cfgNameLabel.TextColor3 = Color3.fromRGB(180, 180, 190)
...
cfgNameBox.Size = UDim2.new(1, -20, 0, 32)
cfgNameBox.Position = UDim2.new(0, 10, 0, 62)
cfgNameBox.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
cfgNameBox.BorderSizePixel = 0
cfgNameBox.Text = ""
cfgNameBox.PlaceholderText = "Enter config name..."
cfgNameBox.PlaceholderColor3 = Color3.fromRGB(100, 100, 110)
...
local cfgNamePad = Instance.new("UIPadding", cfgNameBox)
cfgNamePad.PaddingLeft = UDim.new(0, 8)

-- Status label
local cfgStatusLabel = Instance.new("TextLabel", cfgActionsFrame)
cfgStatusLabel.Size = UDim2.new(1, -20, 0, 20)
cfgStatusLabel.Position = UDim2.new(0, 10, 0, 100)
cfgStatusLabel.BackgroundTransparency = 1
cfgStatusLabel.Text = ""
cfgStatusLabel.TextColor3 = Color3.fromRGB(100, 200, 100)
...
cfgSelectedLabel.Size = UDim2.new(1, -20, 0, 18)
cfgSelectedLabel.Position = UDim2.new(0, 10, 0, 125)
cfgSelectedLabel.BackgroundTransparency = 1
cfgSelectedLabel.Text = "Selected: None"
cfgSelectedLabel.TextColor3 = Color3.fromRGB(150, 130, 220)
cfgSelectedLabel.Font = Enum.Font.GothamBold
cfgSelectedLabel.TextSize = 12
cfgSelectedLabel.TextXAlignment = Enum.TextXAlignment.Left

-- Actions Buttons
local cfgCreateBtn = Instance.new("TextButton", cfgActionsFrame)
cfgCreateBtn.Size = UDim2.new(1, -20, 0, 34)
cfgCreateBtn.Position = UDim2.new(0, 10, 0, 155)
cfgCreateBtn.BackgroundColor3 = Color3.fromRGB(60, 180, 100)
cfgCreateBtn.BorderSizePixel = 0
cfgCreateBtn.Text = "✚ Create"
cfgCreateBtn.TextColor3 = Color3.new(1, 1, 1)
cfgCreateBtn.Font = Enum.Font.GothamBold
cfgCreateBtn.TextSize = 14
cfgCreateBtn.AutoButtonColor = false
Instance.new("UICorner", cfgCreateBtn).CornerRadius = UDim.new(0, 6)

local cfgSaveBtn = Instance.new("TextButton", cfgActionsFrame)
cfgSaveBtn.Size = UDim2.new(1, -20, 0, 34)
cfgSaveBtn.Position = UDim2.new(0, 10, 0, 195)
cfgSaveBtn.BackgroundColor3 = PURPLE
cfgSaveBtn.BorderSizePixel = 0
cfgSaveBtn.Text = "💾 Save"
cfgSaveBtn.TextColor3 = Color3.new(1, 1, 1)
cfgSaveBtn.Font = Enum.Font.GothamBold
cfgSaveBtn.TextSize = 14
cfgSaveBtn.AutoButtonColor = false
Instance.new("UICorner", cfgSaveBtn).CornerRadius = UDim.new(0, 6)

local cfgDeleteBtn = Instance.new("TextButton", cfgActionsFrame)
cfgDeleteBtn.Size = UDim2.new(1, -20, 0, 34)
cfgDeleteBtn.Position = UDim2.new(0, 10, 0, 235)
cfgDeleteBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
cfgDeleteBtn.BorderSizePixel = 0
cfgDeleteBtn.Text = "🗑 Delete"
cfgDeleteBtn.TextColor3 = Color3.new(1, 1, 1)
cfgDeleteBtn.Font = Enum.Font.GothamBold
cfgDeleteBtn.TextSize = 14
cfgDeleteBtn.AutoButtonColor = false
Instance.new("UICorner", cfgDeleteBtn).CornerRadius = UDim.new(0, 6)

local cfgLoadBtn = Instance.new("TextButton", cfgActionsFrame)
cfgLoadBtn.Size = UDim2.new(1, -20, 0, 34)
cfgLoadBtn.Position = UDim2.new(0, 10, 0, 275)
cfgLoadBtn.BackgroundColor3 = Color3.fromRGB(60, 140, 220)
cfgLoadBtn.BorderSizePixel = 0
cfgLoadBtn.Text = "📂 Load"
cfgLoadBtn.TextColor3 = Color3.new(1, 1, 1)
cfgLoadBtn.Font = Enum.Font.GothamBold
cfgLoadBtn.TextSize = 14
cfgLoadBtn.AutoButtonColor = false
...
local function applyHover(btn, baseCol)
    btn.MouseEnter:Connect(function()
        btn.BackgroundColor3 = Color3.fromRGB(
            math.min(baseCol.R * 255 + 25, 255),
            math.min(baseCol.G * 255 + 25, 255),
            math.min(baseCol.B * 255 + 25, 255)
        )
    end)
...
local function updateConfigCount()
    local c = 0
    for _ in pairs(CONFIG_STORE) do c = c + 1 end
...
local function selectConfig(name)
    ...
    for cfgName, data in pairs(CONFIG_LIST_BUTTONS) do
...
local function addConfigToList(name)
    if CONFIG_LIST_BUTTONS[name] then return end
    ...
    nl.Size = UDim2.new(1, -45, 0, 20)
    nl.Position = UDim2.new(0, 40, 0.5, -10)
    ...
    btn.MouseEnter:Connect(function()
        if selectedConfigName ~= name then btn.BackgroundColor3 = Color3.fromRGB(50, 50, 65) end
    end)
    btn.MouseLeave:Connect(function()
        if selectedConfigName ~= name then btn.BackgroundColor3 = Color3.fromRGB(40, 40, 50) end
    end)
    btn.MouseButton1Click:Connect(function()
        playClick()
        selectConfig(name)
    ...
    CONFIG_LIST_BUTTONS[name] = {btn = btn}
    updateConfigCount()
end

local function removeConfigFromList(name)
    if CONFIG_LIST_BUTTONS[name] then
        pcall(function() CONFIG_LIST_BUTTONS[name].btn:Destroy() end)
        CONFIG_LIST_BUTTONS[name] = nil
    end
