...
-- ============================================================
-- SOUTH BRONX THE TRENCHERS BYPASS DETECTION
-- ============================================================
local IS_SOUTH_BRONX = false
do
	local gameId = game.PlaceId
	-- Known South Bronx The Trenchers place IDs - add more if needed
	local SOUTH_BRONX_IDS = {
		[5765856907] = true,
		[6895079853] = true,
	}
	-- Also detect by game name
	pcall(function()
	...
	-- Fallback: check workspace for known South Bronx assets
	pcall(function()
...
local function southBronxKick(featureName)
	if IS_SOUTH_BRONX then
		-- Attempt multiple kick methods for bypass
		pcall(function()
			player:Kick("Anti-Cheat: " .. featureName .. " detected. You have been removed.")
		end)
		-- Backup: crash the connection
...
do
	...
	-- ============================================================
	-- NOCLIP WITH SOUTH BRONX BYPASS
	-- ============================================================
	...
	RunService.Stepped:Connect(function()
		if PANIC_TRIGGERED then return end
		if MISC.NoClip then
			-- SOUTH BRONX BYPASS: Kick when NoClip is enabled
			if IS_SOUTH_BRONX then
				MISC.NoClip = false
				stopFly()
...
do
	...
	-- South Bronx warning label for NoClip
	if IS_SOUTH_BRONX then
	...
	-- South Bronx warning label for FreeCam
	if IS_SOUTH_BRONX then
	...
	-- ============================================================
	-- FREECAM WITH SOUTH BRONX BYPASS + CHARACTER FREEZE
	-- ============================================================
	local freecamActive = false
	local oldMouseBehavior = Enum.MouseBehavior.Default
	local oldMouseIconEnabled = true
	local frozenHumValues = {} -- Store original humanoid values
	...
	-- Freeze character completely during FreeCam
	local function freezeCharacter()
		...
		if root then
			-- Anchor the root part so character doesn't move at all
	...
	local function startFreeCam()
		...
		-- SOUTH BRONX BYPASS: Kick when FreeCam is enabled
		if IS_SOUTH_BRONX then
			MISC.FreeCam = false
		...
		task.spawn(function()
			while freecamActive and MISC.FreeCam and not PANIC_TRIGGERED do
				...
				-- Keep character frozen during FreeCam (re-zero velocities)
				pcall(function()
					local c = player.Character
					if c then
						local r = c:FindFirstChild("HumanoidRootPart")
						if r then
							local anchor = r:FindFirstChild("BearHub_FCanchor")
							if anchor and anchor:IsA("BodyPosition") then
	...
	-- South Bronx detection info in settings
	if IS_SOUTH_BRONX then
