...
_G.BearHub_CLICK_SOUND_ID = "rbxassetid://6895079853"
_G.BearHub_SLIDER_SOUND_ID = "rbxassetid://5765856907"
_G.BearHub_DRAG_SOUND_ID = "rbxassetid://5765856907"
_G.BearHub_PURPLE = Color3.fromRGB(100, 70, 200)
_G.BearHub_GRAY = Color3.fromRGB(60, 60, 70)
_G.BearHub_DARK = Color3.fromRGB(35, 35, 42)
_G.BearHub_CHECK_ICON = "rbxassetid://6031094667"
_G.BearHub_BEAR_ICON = "rbxassetid://7733658504"

local PURPLE = _G.BearHub_PURPLE
local GRAY = _G.BearHub_GRAY
local DARK = _G.BearHub_DARK
...
local ESP = _G.BearHub_ESP

_G.BearHub_FOV_SCALE_TRIGGER = 1
_G.BearHub_FOV_SCALE_AIMBOT = 3
local FOV_SCALE_TRIGGER = 1
local FOV_SCALE_AIMBOT = 3

_G.BearHub_TRIGGERBOT = {
...
local TRIGGERBOT = _G.BearHub_TRIGGERBOT
...
local AIMBOT = _G.BearHub_AIMBOT

_G.BearHub_HITBOX = {Enabled = false, Bone = "Head", Size = 0}
local HITBOX = _G.BearHub_HITBOX
...
local MISC = _G.BearHub_MISC

_G.BearHub_SPECTATE = {Target = nil, Active = false}
local SPECTATE = _G.BearHub_SPECTATE
_G.BearHub_PANIC_TRIGGERED = false
_G.BearHub_mbHeld = {[1]=false,[2]=false,[3]=false,[4]=false,[5]=false}
local mbHeld = _G.BearHub_mbHeld

--============================================================
-- FILE SYSTEM
--============================================================
_G.BearHub_FS_AVAILABLE = false
do
    local ok = pcall(function()
        _G.BearHub_writefile = writefile or (syn and syn.writefile) or (getgenv and getgenv().writefile)
        _G.BearHub_readfile = readfile or (syn and syn.readfile) or (getgenv and getgenv().readfile)
        _G.BearHub_listfiles = listfiles or (syn and syn.listfiles) or (getgenv and getgenv().listfiles)
        _G.BearHub_isfile = isfile or (syn and syn.isfile) or (getgenv and getgenv().isfile)
        _G.BearHub_isfolder = isfolder or (syn and syn.isfolder) or (getgenv and getgenv().isfolder)
        _G.BearHub_makefolder = makefolder or (syn and syn.makefolder) or (getgenv and getgenv().makefolder)
        _G.BearHub_delfile = delfile or (syn and syn.delfile) or (getgenv and getgenv().delfile)
    end)
    if ok and _G.BearHub_writefile and _G.BearHub_readfile and _G.BearHub_listfiles then
        _G.BearHub_FS_AVAILABLE = true
    end
end

_G.BearHub_CONFIG_FOLDER = "BearHub_Configs"
if _G.BearHub_FS_AVAILABLE then
    pcall(function()
        if _G.BearHub_isfolder and not _G.BearHub_isfolder(_G.BearHub_CONFIG_FOLDER) then
            if _G.BearHub_makefolder then _G.BearHub_makefolder(_G.BearHub_CONFIG_FOLDER) end
        end
    end)
end

_G.BearHub_sanitizeFileName = function(name)
...
_G.BearHub_getConfigPath = function(name)
    return _G.BearHub_CONFIG_FOLDER .. "/" .. _G.BearHub_sanitizeFileName(name) .. ".json"
end

--============================================================
-- SOUTH BRONX DETECTION
--============================================================
_G.BearHub_IS_SOUTH_BRONX = false
do
    local gameId = game.PlaceId
    local SOUTH_BRONX_IDS = {[5765856907]=true,[6895079853]=true}
    pcall(function()
        local info = game:GetService("MarketplaceService"):GetProductInfo(gameId)
        if info and info.Name then
            local n = info.Name:lower()
            if n:find("south bronx") or n:find("trencher") then _G.BearHub_IS_SOUTH_BRONX = true end
        end
    end)
    if SOUTH_BRONX_IDS[gameId] then _G.BearHub_IS_SOUTH_BRONX = true end
    pcall(function()
        if workspace:FindFirstChild("SouthBronx") or workspace:FindFirstChild("Trenchers") or workspace:FindFirstChild("TrenchMap") then _G.BearHub_IS_SOUTH_BRONX = true end
        for _, v in ipairs(workspace:GetChildren()) do
            if v.Name:lower():find("bronx") or v.Name:lower():find("trench") then _G.BearHub_IS_SOUTH_BRONX = true; break end
        end
    end)
end
local IS_SOUTH_BRONX = _G.BearHub_IS_SOUTH_BRONX

_G.BearHub_southBronxKick = function(featureName)
    if _G.BearHub_IS_SOUTH_BRONX then
        pcall(function() player:Kick("Anti-Cheat: " .. featureName .. " detected.") end)
        task.delay(0.5, function() pcall(function() while true do RunService.Heartbeat:Wait(); Instance.new("Part", workspace):Destroy() end end) end)
        return true
    end
    return false
end
...
_G.BearHub_espGui = Instance.new("ScreenGui")
_G.BearHub_espGui.Name = "BearHub_ESP"; _G.BearHub_espGui.ResetOnSpawn = false
_G.BearHub_espGui.IgnoreGuiInset = true; _G.BearHub_espGui.DisplayOrder = 100
_G.BearHub_espGui.Parent = playerGui
local espGui = _G.BearHub_espGui

_G.BearHub_fovGui = Instance.new("ScreenGui")
_G.BearHub_fovGui.Name = "BearHub_FOV"; _G.BearHub_fovGui.ResetOnSpawn = false
_G.BearHub_fovGui.IgnoreGuiInset = true; _G.BearHub_fovGui.DisplayOrder = 99
_G.BearHub_fovGui.Parent = playerGui
local fovGui = _G.BearHub_fovGui

_G.BearHub_gui = Instance.new("ScreenGui")
_G.BearHub_gui.Name = "BearHub"; _G.BearHub_gui.ResetOnSpawn = false
_G.BearHub_gui.IgnoreGuiInset = true; _G.BearHub_gui.DisplayOrder = 9999
_G.BearHub_gui.Parent = playerGui
local gui = _G.BearHub_gui

--============================================================
-- PANIC
--============================================================
_G.BearHub_PANIC_DESTROY = function()
    if _G.BearHub_PANIC_TRIGGERED then return end
    _G.BearHub_PANIC_TRIGGERED = true
...
do
    ...
    _G.BearHub_playClick = function() if not _G.BearHub_PANIC_TRIGGERED then playSound(_G.BearHub_CLICK_SOUND_ID, 0.25, 1.2) end end
    _G.BearHub_playSlider = function() if not _G.BearHub_PANIC_TRIGGERED then playSound(_G.BearHub_SLIDER_SOUND_ID, 0.15, 1.5) end end
    _G.BearHub_doClick = function()
        if _G.BearHub_PANIC_TRIGGERED then return false end
        local ok = false
        pcall(function()
    ...
    _G.BearHub_startDragSound = function()
        if _G.BearHub_PANIC_TRIGGERED or dragSoundPlaying then return end
        dragSoundPlaying = true
        dragSoundObj = Instance.new("Sound")
        dragSoundObj.SoundId = _G.BearHub_DRAG_SOUND_ID; dragSoundObj.Volume = 0.12
...
do
    ...
    _G.BearHub_startSpectate = function(target)
        if _G.BearHub_PANIC_TRIGGERED or not target or not target.Character then return end
    ...
    _G.BearHub_stopSpectate = function()
    ...
    _G.BearHub_teleportTo = function(target)
        if _G.BearHub_PANIC_TRIGGERED then return false, "Disabled" end
        ...
        task.spawn(function()
            ...
            while tick() - startTime < 0.5 do
                if _G.BearHub_PANIC_TRIGGERED or not myChar.Parent or not myRoot.Parent or not target.Character then break end
    ...
    _G.BearHub_bringPlayer = function(target)
        ...
        task.spawn(function()
            for i = 1, 5 do
                if _G.BearHub_PANIC_TRIGGERED or not target.Character then break end
    ...
    _G.BearHub_switchPlaces = function(target)
        ...
        task.spawn(function()
            ...
            local startTime = tick()
            while tick() - startTime < 0.5 do
                if _G.BearHub_PANIC_TRIGGERED or not myChar.Parent or not target.Character then break end
    ...
    Players.PlayerRemoving:Connect(function(p) if SPECTATE.Target == p then _G.BearHub_stopSpectate() end end)
end
...
UIS.InputBegan:Connect(function(inp, gameProcessed)
    if _G.BearHub_PANIC_TRIGGERED or gameProcessed then return end
...
do
    ...
    local function createESPData(plr)
        ...
        local d = {holder=h,boxTop=makeLine(h),boxBot=makeLine(h),boxLeft=makeLine(h),boxRight=makeLine(h),skeleton={},snapline=makeLine(h),healthBg=makeLine(h),healthFill=makeLine(h),name=makeText(h,14),id=makeText(h,12),distance=makeText(h,12),inventory=makeText(h,11),inventory2=makeText(h,11)}
    ...
    _G.BearHub_fullRefresh = function() for plr in pairs(espObjects) do clearESP(plr) end end
    ...
    _G.BearHub_getPos = function(char, name)
        local p = char:FindFirstChild(name)
        if p and p:IsA("BasePart") then return p.Position end; return nil
    end
    _G.BearHub_visCheck = function(tp, tc)
    ...
    local function getCachedInv(plr)
        ...
        if plr.Character then
            for _, c in ipairs(plr.Character:GetChildren()) do
                if c:IsA("Tool") and not seen[c.Name] then seen[c.Name] = true; table.insert(items, c.Name) end
            end
        end
                        ...
                        if not skip then
                            ...
                            if not skip then
                                if ESP.VisibleOnly and plr ~= player then
                                    if not _G.BearHub_visCheck(root.Position, char) then if d then hideAll(d) end; skip = true end
                                end
                                if not skip then
                                    ...
                                    if ESP.Box.Enabled then
                                        ...
                                        for _, f in ipairs({d.boxTop,d.boxBot,d.boxLeft,d.boxRight}) do
                                    ...
                                    if ESP.Inventory.Enabled then
                                        ...
                                        if #items > 0 then
                                            ...
                                            d.inventory.Text = "[" .. table.concat(row1, ", ") .. "]"
                                            ...
                                            d.inventory.Visible = true; bo = bo + 16
                                            if #row2 > 0 then
                                                d.inventory2.Text = "[" .. table.concat(row2, ", ") .. "]"
                                                ...
                                                d.inventory2.Visible = true; bo = bo + 16
                                            ...
                                            d.inventory.Visible = true; d.inventory2.Visible = false; bo = bo + 16
                                        end
                                    ...
                                    if ESP.Skeleton.Enabled then
                                        ...
                                        for i = 1, 12 do
                                            if d.skeleton[i] then
                                                if i <= #bones then
                                                    local a = _G.BearHub_getPos(char, bones[i][1])
                                                    local b = _G.BearHub_getPos(char, bones[i][2])
                                                    if a and b then
    ...
    RunService.RenderStepped:Connect(function()
        if _G.BearHub_PANIC_TRIGGERED then return end
        pcall(updateESP); pcall(updateFOVCircle)
    end)
    task.spawn(function() while true do task.wait(5); if _G.BearHub_PANIC_TRIGGERED then break end; pcall(_G.BearHub_fullRefresh) end end)
    player.CharacterAdded:Connect(function() task.wait(0.5); if not _G.BearHub_PANIC_TRIGGERED then pcall(_G.BearHub_fullRefresh) end end)
    workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
        Camera = workspace.CurrentCamera; if not _G.BearHub_PANIC_TRIGGERED then pcall(_G.BearHub_fullRefresh) end
    end)
...
do
    ...
    local function getTriggerTarget()
        if not TRIGGERBOT.Enabled or _G.BearHub_PANIC_TRIGGERED then return nil end
                ...
                if char then
                    ...
                    if hum and head and root then
                        ...
                        if dc and TRIGGERBOT.VisibleOnly then
                            if not _G.BearHub_visCheck(root.Position, char) then dc = false end
                        end
                        if dc then
    ...
    task.spawn(function()
        while true do
            task.wait(0.05); if _G.BearHub_PANIC_TRIGGERED then break end
    ...
    local function getBonePosition(char, boneChoice)
        if boneChoice == "Head" then return _G.BearHub_getPos(char, "Head")
        elseif boneChoice == "Torso" then return _G.BearHub_getPos(char, "UpperTorso") or _G.BearHub_getPos(char, "Torso") or _G.BearHub_getPos(char, "HumanoidRootPart")
        elseif boneChoice == "Legs" then return _G.BearHub_getPos(char, "LeftUpperLeg") or _G.BearHub_getPos(char, "Left Leg") or _G.BearHub_getPos(char, "LowerTorso") end
        return _G.BearHub_getPos(char, "Head")
    end
    local function getAimbotTarget()
        if not AIMBOT.Enabled or _G.BearHub_PANIC_TRIGGERED then return nil end
        local vc = Camera.ViewportSize / 2
                ...
                if char then
                    ...
                    if hum and root then
                        ...
                        if dc then
                            ...
                            if bonePos then
                                if AIMBOT.VisibleCheck and not _G.BearHub_visCheck(bonePos, char) then dc = false end
                                if dc then
                                    local ok, sp, on, d2 = pcall(function()
...
--============================================================
-- MISC (SemiGod, WalkSpeed, JumpPower, NoClip, Tools)
--============================================================
do
    task.spawn(function()
        while true do
            task.wait(0.1); if _G.BearHub_PANIC_TRIGGERED then break end
    ...
    local function hookHum(c)
        ...
        h.HealthChanged:Connect(function()
            if MISC.SemiGod and not _G.BearHub_PANIC_TRIGGERED then
    ...
    local function startFly()
        ...
        task.spawn(function()
            while flying and MISC.NoClip and not _G.BearHub_PANIC_TRIGGERED do
    ...
    RunService.Stepped:Connect(function()
        ...
        if MISC.NoClip then
            if IS_SOUTH_BRONX then
                MISC.NoClip = false; stopFly(); _G.BearHub_southBronxKick("NoClip"); return
            end
            local c = player.Character
            if c then
    ...
    task.spawn(function()
        while true do
            task.wait(3); if _G.BearHub_PANIC_TRIGGERED then break end
    ...
    local function doSuperPunch()
        if _G.BearHub_PANIC_TRIGGERED or not MISC.SuperPunch then return end
        ...
        task.spawn(function()
            for i = 1, 100 do
                if _G.BearHub_PANIC_TRIGGERED or not MISC.SuperPunch or not best.Parent then break end
    ...
    local function hookToolSP(tool)
        ...
        local conn; conn = tool.Activated:Connect(function()
            if MISC.SuperPunch and not _G.BearHub_PANIC_TRIGGERED then doSuperPunch() end
        end)
    ...
    task.spawn(function() while true do task.wait(2); if _G.BearHub_PANIC_TRIGGERED then break end; pcall(scanSP) end end)
    ...
    task.spawn(function()
        while true do
            if _G.BearHub_PANIC_TRIGGERED then break end
...
-- CONFIG SYSTEM
--============================================================
_G.BearHub_CONFIG_STORE = {}
_G.BearHub_CONFIG_LIST_BUTTONS = {}
_G.BearHub_selectedConfigName = nil
_G.BearHub_UI_REGISTRY = {checks={}, colorPickers={}, sliders={}, dropdowns={}, keybinds={}}

_G.BearHub_colorToTable = function(c) return {R=c.R,G=c.G,B=c.B} end
_G.BearHub_tableToColor = function(t)
    if type(t) ~= "table" then return Color3.new(1,1,1) end
    return Color3.new(t.R or 1, t.G or 1, t.B or 1)
end

_G.BearHub_serializeCurrentConfig = function()
    return {
        ESP = {Enabled=ESP.Enabled,MaxDistance=ESP.MaxDistance,ShowLocalPlayer=ESP.ShowLocalPlayer,VisibleOnly=ESP.VisibleOnly,
            Box={Enabled=ESP.Box.Enabled,Color=_G.BearHub_colorToTable(ESP.Box.Color)},
            Skeleton={Enabled=ESP.Skeleton.Enabled,Color=_G.BearHub_colorToTable(ESP.Skeleton.Color)},
            Name={Enabled=ESP.Name.Enabled,Color=_G.BearHub_colorToTable(ESP.Name.Color)},
            ID={Enabled=ESP.ID.Enabled,Color=_G.BearHub_colorToTable(ESP.ID.Color)},
            HealthBar={Enabled=ESP.HealthBar.Enabled,Color=_G.BearHub_colorToTable(ESP.HealthBar.Color)},
            Distance={Enabled=ESP.Distance.Enabled,Color=_G.BearHub_colorToTable(ESP.Distance.Color)},
            Snaplines={Enabled=ESP.Snaplines.Enabled,Color=_G.BearHub_colorToTable(ESP.Snaplines.Color)},
            Inventory={Enabled=ESP.Inventory.Enabled,Color=_G.BearHub_colorToTable(ESP.Inventory.Color)}},
        TRIGGERBOT = {Enabled=TRIGGERBOT.Enabled,KeybindName=TRIGGERBOT.KeybindName,Type=TRIGGERBOT.Type,ShowFOV=TRIGGERBOT.ShowFOV,FOVColor=_G.BearHub_colorToTable(TRIGGERBOT.FOVColor),FOV=TRIGGERBOT.FOV,ExcludeDead=TRIGGERBOT.ExcludeDead,VisibleOnly=TRIGGERBOT.VisibleOnly,MaxDistance=TRIGGERBOT.MaxDistance,ShotDelay=TRIGGERBOT.ShotDelay},
        AIMBOT = {Enabled=AIMBOT.Enabled,KeybindName=AIMBOT.KeybindName,DrawFOV=AIMBOT.DrawFOV,FOVColor=_G.BearHub_colorToTable(AIMBOT.FOVColor),VisibleCheck=AIMBOT.VisibleCheck,ExcludeDead=AIMBOT.ExcludeDead,Bone=AIMBOT.Bone,FOV=AIMBOT.FOV,MaxDistance=AIMBOT.MaxDistance,SmoothX=AIMBOT.SmoothX,SmoothY=AIMBOT.SmoothY},
        HITBOX = {Enabled=HITBOX.Enabled,Bone=HITBOX.Bone,Size=HITBOX.Size},
        MISC = {SemiGod=MISC.SemiGod,NoRecoil=MISC.NoRecoil,NoSpread=MISC.NoSpread,InfAmmo=MISC.InfAmmo,SuperPunch=MISC.SuperPunch,NoClip=MISC.NoClip,NoClipSpeed=MISC.NoClipSpeed,RapidFire=MISC.RapidFire,RapidFireLevel=MISC.RapidFireLevel,WalkSpeedEnabled=MISC.WalkSpeedEnabled,WalkSpeed=MISC.WalkSpeed,JumpPowerEnabled=MISC.JumpPowerEnabled,JumpPower=MISC.JumpPower,FreeCam=MISC.FreeCam,FreeCamSpeed=MISC.FreeCamSpeed}
    }
end

_G.BearHub_loadConfigData = function(data)
    ...
    if data.ESP then
        ...
        for _, name in ipairs({"Box","Skeleton","Name","ID","HealthBar","Distance","Snaplines","Inventory"}) do
            if data.ESP[name] then
                if data.ESP[name].Enabled ~= nil then ESP[name].Enabled = data.ESP[name].Enabled end
                if data.ESP[name].Color then ESP[name].Color = _G.BearHub_tableToColor(data.ESP[name].Color) end
            end
        end
    end
    if data.TRIGGERBOT then
        for k, v in pairs(data.TRIGGERBOT) do
            if TRIGGERBOT[k] ~= nil and v ~= nil then
                if k == "FOVColor" then TRIGGERBOT[k] = _G.BearHub_tableToColor(v)
                else TRIGGERBOT[k] = v end
            end
        end
    ...
    if data.AIMBOT then
        for k, v in pairs(data.AIMBOT) do
            if AIMBOT[k] ~= nil and v ~= nil then
                if k == "FOVColor" then AIMBOT[k] = _G.BearHub_tableToColor(v)
                else AIMBOT[k] = v end
            end
        end
...
_G.BearHub_saveConfigToFile = function(name, data)
    if not _G.BearHub_FS_AVAILABLE then return false, "FS not available" end
    local path = _G.BearHub_getConfigPath(name)
    local ok, err = pcall(function()
        _G.BearHub_writefile(path, HttpService:JSONEncode(data))
    end)
    if ok then return true end
    return false, tostring(err)
end

_G.BearHub_deleteConfigFile = function(name)
    if not _G.BearHub_FS_AVAILABLE then return false end
    local path = _G.BearHub_getConfigPath(name)
    return pcall(function()
        if _G.BearHub_isfile and _G.BearHub_isfile(path) and _G.BearHub_delfile then
            _G.BearHub_delfile(path)
        end
    end)
end

_G.BearHub_scanConfigFolder = function()
    if not _G.BearHub_FS_AVAILABLE then return {} end
    local found = {}
    pcall(function()
        if _G.BearHub_isfolder and _G.BearHub_isfolder(_G.BearHub_CONFIG_FOLDER) then
            local files = _G.BearHub_listfiles(_G.BearHub_CONFIG_FOLDER)
            if files then
                for _, filepath in ipairs(files) do
                    local ok, name = pcall(function() return filepath:match("([^/\\]+)%.json$") end)
                    if ok and name then
                        local content = nil
                        pcall(function() content = _G.BearHub_readfile(filepath) end)
                        if content then
                            local ok2, data = pcall(function() return HttpService:JSONDecode(content) end)
                            if ok2 and data then found[name] = data end
                        end
                    end
                end
            end
...
print("[BearHub] Part 1 loaded! Now execute Part 2 for the GUI.")
-- Sprawdzenie czy Część 1 została załadowana
if not _G.BearHub_ESP then
    warn("[BearHub] BŁĄD: Część 1 nie została załadowana! Wykonaj najpierw Część 1.")
    return
end
...
local GRAY = _G.BearHub_GRAY
local DARK = _G.BearHub_DARK
local CHECK_ICON = _G.BearHub_CHECK_ICON
local BEAR_ICON = _G.BearHub_BEAR_ICON
local ESP = _G.BearHub_ESP
local TRIGGERBOT = _G.BearHub_TRIGGERBOT
local AIMBOT = _G.BearHub_AIMBOT
local HITBOX = _G.BearHub_HITBOX
...
local mbHeld = _G.BearHub_mbHeld
local IS_SOUTH_BRONX = _G.BearHub_IS_SOUTH_BRONX
local FS_AVAILABLE = _G.BearHub_FS_AVAILABLE
...
local southBronxKick = _G.BearHub_southBronxKick
...
do
    ...
    mkCheck = function(p,t,tbl,k,o)
        ...
        table.insert(_G.BearHub_UI_REGISTRY.checks, {box=box, checkIcon=ck, tbl=tbl, key=k, isSub=false})
    end
    ...
    mkCheckColor = function(p,t,tbl,k,ck2,o)
        ...
        table.insert(_G.BearHub_UI_REGISTRY.checks, {box=box, checkIcon=ck, tbl=tbl, key=k, isSub=isSub})
        table.insert(_G.BearHub_UI_REGISTRY.colorPickers, {colorIndicator=ci, tbl=tbl, key=k, isSub=isSub, subKey=ck2})
    end
    ...
    mkSlider = function(p,t,minV,maxV,def,suf,tbl,k,o)
        ...
        table.insert(_G.BearHub_UI_REGISTRY.sliders, {fill=fill, knob=knob, label=vl, min=minV, max=maxV, suf=suf, tbl=tbl, key=k})
    end
    ...
    mkDropdown = function(p,t,opts,tbl,k,o)
        ...
        table.insert(_G.BearHub_UI_REGISTRY.dropdowns, {button=btn, tbl=tbl, key=k})
    end
    ...
    mkKeybind = function(p,t,tbl,o)
        ...
        table.insert(_G.BearHub_UI_REGISTRY.keybinds, {box=box, checkIcon=ck, keyBtn=keyBtn, tbl=tbl, bindLookup=BIND_LOOKUP})
    end
...
--============================================================
-- REFRESH ALL UI (for config load)
--============================================================
local function refreshAllUI()
    for _, data in ipairs(_G.BearHub_UI_REGISTRY.checks) do
        pcall(function()
            local val
            if data.isSub then val = ESP[data.key] and ESP[data.key].Enabled
            else val = data.tbl[data.key] end
            val = val or false
            data.box.BackgroundColor3 = val and PURPLE or GRAY
            data.checkIcon.Visible = val
        end)
    end
    for _, data in ipairs(_G.BearHub_UI_REGISTRY.colorPickers) do
        pcall(function()
            local col
            if data.isSub then col = ESP[data.key] and ESP[data.key].Color
            else col = data.tbl[data.subKey] end
            if col then data.colorIndicator.BackgroundColor3 = col end
        end)
    end
    for _, data in ipairs(_G.BearHub_UI_REGISTRY.sliders) do
        pcall(function()
    ...
    for _, data in ipairs(_G.BearHub_UI_REGISTRY.dropdowns) do
        pcall(function() data.button.Text = "  " .. tostring(data.tbl[data.key]) end)
    end
    for _, data in ipairs(_G.BearHub_UI_REGISTRY.keybinds) do
        pcall(function()
...
do
    ...
    if IS_SOUTH_BRONX then
        local warnLbl = Instance.new("TextLabel", mvPanel)
        warnLbl.Size = UDim2.new(1, 0, 0, 30); warnLbl.BackgroundTransparency = 1
        warnLbl.Text = "⚠️ South Bronx: NoClip = KICK"; warnLbl.TextColor3 = Color3.fromRGB(255, 80, 80)
        warnLbl.Font = Enum.Font.GothamBold; warnLbl.TextSize = 12
        warnLbl.TextWrapped = true; warnLbl.LayoutOrder = 8
    end
    ...
    if IS_SOUTH_BRONX then
        local warnLbl2 = Instance.new("TextLabel", fcPanel)
        warnLbl2.Size = UDim2.new(1, 0, 0, 30); warnLbl2.BackgroundTransparency = 1
        warnLbl2.Text = "⚠️ South Bronx: FreeCam = KICK"; warnLbl2.TextColor3 = Color3.fromRGB(255, 80, 80)
        warnLbl2.Font = Enum.Font.GothamBold; warnLbl2.TextSize = 12
        warnLbl2.TextWrapped = true; warnLbl2.LayoutOrder = 4
    end
    ...
    fcTpBtn.Font = Enum.Font.GothamBold; fcTpBtn.TextSize = 12
    fcTpBtn.AutoButtonColor = false; fcTpBtn.ZIndex = 9999
    ...
    local function freezeCharacter()
        ...
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            frozenHumValues.WalkSpeed = hum.WalkSpeed
            frozenHumValues.JumpPower = hum.JumpPower
            frozenHumValues.JumpHeight = hum.JumpHeight
            hum.WalkSpeed = 0; hum.JumpPower = 0; hum.JumpHeight = 0
        end
        if root then
            local anchor = Instance.new("BodyPosition")
            anchor.Name = "BearHub_FCanchor"
            anchor.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
            anchor.D = 5000; anchor.P = 50000
            ...
            bvel.Velocity = Vector3.zero; bvel.Parent = root
        end
    end
    ...
    local function unfreezeCharacter()
        ...
        if hum then
            hum.WalkSpeed = frozenHumValues.WalkSpeed or (MISC.WalkSpeedEnabled and MISC.WalkSpeed or 16)
            hum.JumpPower = frozenHumValues.JumpPower or (MISC.JumpPowerEnabled and MISC.JumpPower or 50)
            hum.JumpHeight = frozenHumValues.JumpHeight or 7.2
        end
        frozenHumValues = {}
    end
    ...
    local function freecamTeleport()
        if not freecamActive or _G.BearHub_PANIC_TRIGGERED then return end
        local char = player.Character; if not char then return end
        ...
        else targetPos = camPos + camLook * 50 end
    ...
    local function startFreeCam()
        if freecamActive or _G.BearHub_PANIC_TRIGGERED then return end
        if IS_SOUTH_BRONX then MISC.FreeCam = false; southBronxKick("FreeCam"); return end
        local char = player.Character; if not char then return end
        local root = char:FindFirstChild("HumanoidRootPart"); if not root then return end
        ...
        task.spawn(function()
            while freecamActive and MISC.FreeCam and not _G.BearHub_PANIC_TRIGGERED do
                ...
                pcall(function() Camera.CFrame = CFrame.new(camPos) * rotCF end)
                pcall(function()
                    local c = player.Character
                    if c then
                        local h = c:FindFirstChildOfClass("Humanoid")
    ...
    UIS.InputBegan:Connect(function(inp, gp)
        if _G.BearHub_PANIC_TRIGGERED or gp then return end
    ...
    task.spawn(function() while true do task.wait(0.5); if _G.BearHub_PANIC_TRIGGERED then break end; if selPl and selPl.Parent then local d=getDist(selPl); plDL.Text=d and ("Distance: "..d.."m") or "Distance: N/A" end end end)
    ...
    Players.PlayerAdded:Connect(function(plr) task.wait(0.5); if plr~=player and plr.Parent and not _G.BearHub_PANIC_TRIGGERED then pcall(refreshPL) end end)
    ...
    task.spawn(function() while true do task.wait(3); if _G.BearHub_PANIC_TRIGGERED then break end; pcall(refreshPL) end end)
    ...
    if IS_SOUTH_BRONX then
        local sbInfo = Instance.new("TextLabel", sgPanel)
        sbInfo.Size = UDim2.new(1, 0, 0, 25); sbInfo.BackgroundTransparency = 1
        sbInfo.Text = "🎯 South Bronx: NoClip/FreeCam = KICK"
        sbInfo.TextColor3 = Color3.fromRGB(255, 120, 60); sbInfo.Font = Enum.Font.GothamBold
        sbInfo.TextSize = 11; sbInfo.TextWrapped = true; sbInfo.LayoutOrder = 3
    end
    ...
    local cfgListFrame = Instance.new("Frame", stConfigP)
    cfgListFrame.Size = UDim2.new(0.42, 0, 1, -10); cfgListFrame.Position = UDim2.new(0, 10, 0, 5)
    cfgListFrame.BackgroundColor3 = DARK; cfgListFrame.BorderSizePixel = 0
    Instance.new("UICorner", cfgListFrame).CornerRadius = UDim.new(0, 8)

    local cfgListTitle = Instance.new("TextLabel", cfgListFrame)
    cfgListTitle.Size = UDim2.new(1, -10, 0, 25); cfgListTitle.Position = UDim2.new(0, 10, 0, 8)
    cfgListTitle.BackgroundTransparency = 1; cfgListTitle.Text = "Saved Configs"
    cfgListTitle.TextColor3 = Color3.fromRGB(160, 160, 170); cfgListTitle.Font = Enum.Font.GothamBold
    cfgListTitle.TextSize = 14; cfgListTitle.TextXAlignment = Enum.TextXAlignment.Left

    local cfgCountLabel = Instance.new("TextLabel", cfgListFrame)
    cfgCountLabel.Size = UDim2.new(0, 80, 0, 25); cfgCountLabel.Position = UDim2.new(1, -85, 0, 8)
    cfgCountLabel.BackgroundTransparency = 1; cfgCountLabel.Text = "0 configs"
    cfgCountLabel.TextColor3 = Color3.fromRGB(120, 120, 130); cfgCountLabel.Font = Enum.Font.Gotham
    cfgCountLabel.TextSize = 11; cfgCountLabel.TextXAlignment = Enum.TextXAlignment.Right

    local cfgScroll = Instance.new("ScrollingFrame", cfgListFrame)
    cfgScroll.Size = UDim2.new(1, -10, 1, -42); cfgScroll.Position = UDim2.new(0, 5, 0, 38)
    cfgScroll.BackgroundTransparency = 1; cfgScroll.ScrollBarThickness = 3
    cfgScroll.ScrollBarImageColor3 = PURPLE; cfgScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
    cfgScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; cfgScroll.BorderSizePixel = 0
    Instance.new("UIListLayout", cfgScroll).Padding = UDim.new(0, 4)

    local cfgActionsFrame = Instance.new("Frame", stConfigP)
    cfgActionsFrame.Size = UDim2.new(0.55, 0, 1, -10); cfgActionsFrame.Position = UDim2.new(0.44, 5, 0, 5)
    cfgActionsFrame.BackgroundColor3 = DARK; cfgActionsFrame.BorderSizePixel = 0
    Instance.new("UICorner", cfgActionsFrame).CornerRadius = UDim.new(0, 8)

    local cfgActTitle = Instance.new("TextLabel", cfgActionsFrame)
    cfgActTitle.Size = UDim2.new(1, -20, 0, 25); cfgActTitle.Position = UDim2.new(0, 10, 0, 8)
    cfgActTitle.BackgroundTransparency = 1; cfgActTitle.Text = "Config Actions"
    cfgActTitle.TextColor3 = Color3.fromRGB(160, 160, 170); cfgActTitle.Font = Enum.Font.GothamBold
    cfgActTitle.TextSize = 14; cfgActTitle.TextXAlignment = Enum.TextXAlignment.Left

    local cfgFSLabel = Instance.new("TextLabel", cfgActionsFrame)
    cfgFSLabel.Size = UDim2.new(0, 130, 0, 25); cfgFSLabel.Position = UDim2.new(1, -140, 0, 8)
    ...
    cfgFSLabel.Font = Enum.Font.GothamBold; cfgFSLabel.TextSize = 11
    cfgFSLabel.TextXAlignment = Enum.TextXAlignment.Right

    local cfgNameLabel = Instance.new("TextLabel", cfgActionsFrame)
    cfgNameLabel.Size = UDim2.new(1, -20, 0, 18); cfgNameLabel.Position = UDim2.new(0, 10, 0, 40)
    cfgNameLabel.BackgroundTransparency = 1; cfgNameLabel.Text = "Config Name"
    cfgNameLabel.TextColor3 = Color3.fromRGB(180, 180, 190); cfgNameLabel.Font = Enum.Font.GothamBold
    cfgNameLabel.TextSize = 12; cfgNameLabel.TextXAlignment = Enum.TextXAlignment.Left

    local cfgNameBox = Instance.new("TextBox", cfgActionsFrame)
    cfgNameBox.Size = UDim2.new(1, -20, 0, 32); cfgNameBox.Position = UDim2.new(0, 10, 0, 62)
    cfgNameBox.BackgroundColor3 = Color3.fromRGB(40, 40, 50); cfgNameBox.BorderSizePixel = 0
    cfgNameBox.Text = ""; cfgNameBox.PlaceholderText = "Enter config name..."
    cfgNameBox.PlaceholderColor3 = Color3.fromRGB(100, 100, 110); cfgNameBox.TextColor3 = Color3.new(1, 1, 1)
    cfgNameBox.Font = Enum.Font.Gotham; cfgNameBox.TextSize = 13; cfgNameBox.ClearTextOnFocus = false
    Instance.new("UICorner", cfgNameBox).CornerRadius = UDim.new(0, 6)
    Instance.new("UIPadding", cfgNameBox).PaddingLeft = UDim.new(0, 8)

    local cfgStatusLabel = Instance.new("TextLabel", cfgActionsFrame)
    cfgStatusLabel.Size = UDim2.new(1, -20, 0, 20); cfgStatusLabel.Position = UDim2.new(0, 10, 0, 100)
    cfgStatusLabel.BackgroundTransparency = 1; cfgStatusLabel.Text = ""
    cfgStatusLabel.TextColor3 = Color3.fromRGB(100, 200, 100); cfgStatusLabel.Font = Enum.Font.GothamBold
    cfgStatusLabel.TextSize = 11; cfgStatusLabel.TextXAlignment = Enum.TextXAlignment.Center
    cfgStatusLabel.TextWrapped = true

    local function cfgStatus(txt, col, dur)
        cfgStatusLabel.Text = txt
        cfgStatusLabel.TextColor3 = col or Color3.fromRGB(100, 200, 100)
        if dur then
            task.spawn(function()
                task.wait(dur)
                if cfgStatusLabel.Text == txt then cfgStatusLabel.Text = "" end
            end)
        end
    end

    local cfgSelectedLabel = Instance.new("TextLabel", cfgActionsFrame)
    cfgSelectedLabel.Size = UDim2.new(1, -20, 0, 18); cfgSelectedLabel.Position = UDim2.new(0, 10, 0, 125)
    cfgSelectedLabel.BackgroundTransparency = 1; cfgSelectedLabel.Text = "Selected: None"
    cfgSelectedLabel.TextColor3 = Color3.fromRGB(150, 130, 220); cfgSelectedLabel.Font = Enum.Font.GothamBold
    cfgSelectedLabel.TextSize = 12; cfgSelectedLabel.TextXAlignment = Enum.TextXAlignment.Left

    local cfgCreateBtn = Instance.new("TextButton", cfgActionsFrame)
    cfgCreateBtn.Size = UDim2.new(1, -20, 0, 34); cfgCreateBtn.Position = UDim2.new(0, 10, 0, 155)
    cfgCreateBtn.BackgroundColor3 = Color3.fromRGB(60, 180, 100); cfgCreateBtn.BorderSizePixel = 0
    cfgCreateBtn.Text = "+ Create"; cfgCreateBtn.TextColor3 = Color3.new(1, 1, 1)
    cfgCreateBtn.Font = Enum.Font.GothamBold; cfgCreateBtn.TextSize = 14; cfgCreateBtn.AutoButtonColor = false
    Instance.new("UICorner", cfgCreateBtn).CornerRadius = UDim.new(0, 6)

    local cfgSaveBtn = Instance.new("TextButton", cfgActionsFrame)
    cfgSaveBtn.Size = UDim2.new(1, -20, 0, 34); cfgSaveBtn.Position = UDim2.new(0, 10, 0, 195)
    cfgSaveBtn.BackgroundColor3 = PURPLE; cfgSaveBtn.BorderSizePixel = 0
    cfgSaveBtn.Text = "Save"; cfgSaveBtn.TextColor3 = Color3.new(1, 1, 1)
    cfgSaveBtn.Font = Enum.Font.GothamBold; cfgSaveBtn.TextSize = 14; cfgSaveBtn.AutoButtonColor = false
    Instance.new("UICorner", cfgSaveBtn).CornerRadius = UDim.new(0, 6)

    local cfgDeleteBtn = Instance.new("TextButton", cfgActionsFrame)
    cfgDeleteBtn.Size = UDim2.new(1, -20, 0, 34); cfgDeleteBtn.Position = UDim2.new(0, 10, 0, 235)
    cfgDeleteBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50); cfgDeleteBtn.BorderSizePixel = 0
    cfgDeleteBtn.Text = "Delete"; cfgDeleteBtn.TextColor3 = Color3.new(1, 1, 1)
    cfgDeleteBtn.Font = Enum.Font.GothamBold; cfgDeleteBtn.TextSize = 14; cfgDeleteBtn.AutoButtonColor = false
    Instance.new("UICorner", cfgDeleteBtn).CornerRadius = UDim.new(0, 6)

    local cfgLoadBtn = Instance.new("TextButton", cfgActionsFrame)
    cfgLoadBtn.Size = UDim2.new(1, -20, 0, 34); cfgLoadBtn.Position = UDim2.new(0, 10, 0, 275)
    cfgLoadBtn.BackgroundColor3 = Color3.fromRGB(60, 140, 220); cfgLoadBtn.BorderSizePixel = 0
    cfgLoadBtn.Text = "Load"; cfgLoadBtn.TextColor3 = Color3.new(1, 1, 1)
    cfgLoadBtn.Font = Enum.Font.GothamBold; cfgLoadBtn.TextSize = 14; cfgLoadBtn.AutoButtonColor = false
    Instance.new("UICorner", cfgLoadBtn).CornerRadius = UDim.new(0, 6)

    local function applyHover(btn, baseCol)
        btn.MouseEnter:Connect(function()
            btn.BackgroundColor3 = Color3.fromRGB(math.min(baseCol.R*255+25, 255), math.min(baseCol.G*255+25, 255), math.min(baseCol.B*255+25, 255))
        end)
        btn.MouseLeave:Connect(function() btn.BackgroundColor3 = baseCol end)
    end
    applyHover(cfgCreateBtn, Color3.fromRGB(60, 180, 100))
    applyHover(cfgSaveBtn, PURPLE)
    applyHover(cfgDeleteBtn, Color3.fromRGB(200, 50, 50))
    applyHover(cfgLoadBtn, Color3.fromRGB(60, 140, 220))

    local function updateConfigCount()
        local c = 0
        for _ in pairs(_G.BearHub_CONFIG_STORE) do c = c + 1 end
        cfgCountLabel.Text = c .. " config" .. (c == 1 and "" or "s")
    end

    local function selectConfig(name)
        _G.BearHub_selectedConfigName = name
        cfgSelectedLabel.Text = "Selected: " .. (name or "None")
        for cfgName, data in pairs(_G.BearHub_CONFIG_LIST_BUTTONS) do
            if data.btn and data.btn.Parent then
                data.btn.BackgroundColor3 = (cfgName == name) and Color3.fromRGB(70, 50, 140) or Color3.fromRGB(40, 40, 50)
            end
        end
    end

    local function addConfigToList(name)
        if _G.BearHub_CONFIG_LIST_BUTTONS[name] then return end
        local btn = Instance.new("TextButton", cfgScroll)
        btn.Size = UDim2.new(1, -6, 0, 38); btn.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
        btn.BorderSizePixel = 0; btn.Text = ""; btn.AutoButtonColor = false
        btn.LayoutOrder = tick() * 1000
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 6)
        local icon = Instance.new("TextLabel", btn)
        icon.Size = UDim2.new(0, 28, 0, 28); icon.Position = UDim2.new(0, 6, 0.5, -14)
        icon.BackgroundColor3 = PURPLE; icon.BorderSizePixel = 0; icon.Text = "F"
        icon.TextSize = 14; icon.Font = Enum.Font.GothamBold; icon.TextColor3 = Color3.new(1, 1, 1)
        Instance.new("UICorner", icon).CornerRadius = UDim.new(0, 6)
        local nl = Instance.new("TextLabel", btn)
        nl.Size = UDim2.new(1, -45, 0, 20); nl.Position = UDim2.new(0, 40, 0.5, -10)
        nl.BackgroundTransparency = 1; nl.Text = name; nl.TextColor3 = Color3.new(1, 1, 1)
        ...
        btn.MouseEnter:Connect(function()
            if _G.BearHub_selectedConfigName ~= name then btn.BackgroundColor3 = Color3.fromRGB(50, 50, 65) end
        end)
        btn.MouseLeave:Connect(function()
            if _G.BearHub_selectedConfigName ~= name then btn.BackgroundColor3 = Color3.fromRGB(40, 40, 50) end
        end)
        btn.MouseButton1Click:Connect(function() playClick(); selectConfig(name) end)
        _G.BearHub_CONFIG_LIST_BUTTONS[name] = {btn = btn}
        updateConfigCount()
    end

    local function removeConfigFromList(name)
        if _G.BearHub_CONFIG_LIST_BUTTONS[name] then
            pcall(function() _G.BearHub_CONFIG_LIST_BUTTONS[name].btn:Destroy() end)
            _G.BearHub_CONFIG_LIST_BUTTONS[name] = nil
        end
        if _G.BearHub_selectedConfigName == name then
            _G.BearHub_selectedConfigName = nil
            cfgSelectedLabel.Text = "Selected: None"
        end
        updateConfigCount()
    end
    ...
    task.spawn(function()
        ...
        if FS_AVAILABLE then
            local found = _G.BearHub_scanConfigFolder()
            for name, data in pairs(found) do
                _G.BearHub_CONFIG_STORE[name] = data
                addConfigToList(name)
            end
            local cnt = 0
            for _ in pairs(_G.BearHub_CONFIG_STORE) do cnt = cnt + 1 end
            if cnt > 0 then
    ...
    cfgCreateBtn.MouseButton1Click:Connect(function()
        ...
        if name == nil or name:match("^%s*$") then
            cfgStatus("Enter a config name!", Color3.fromRGB(255, 100, 100), 2); return
        end
        name = name:match("^%s*(.-)%s*$")
        if #name > 30 then name = name:sub(1, 30) end
        if _G.BearHub_CONFIG_STORE[name] then
            cfgStatus("Config already exists!", Color3.fromRGB(255, 180, 60), 2); return
        end
        local data = _G.BearHub_serializeCurrentConfig()
        _G.BearHub_CONFIG_STORE[name] = data
        addConfigToList(name); selectConfig(name); cfgNameBox.Text = ""
        if FS_AVAILABLE then
            local ok, err = _G.BearHub_saveConfigToFile(name, data)
            if ok then
                cfgStatus("Created '" .. name .. "' (saved)", Color3.fromRGB(100, 220, 120), 2)
            else
                cfgStatus("Created '" .. name .. "' (memory only)", Color3.fromRGB(255, 180, 60), 3)
            end
        else
            cfgStatus("Created '" .. name .. "' (no file support)", Color3.fromRGB(255, 180, 60), 3)
        end
    end)

    cfgSaveBtn.MouseButton1Click:Connect(function()
        playClick()
        if not _G.BearHub_selectedConfigName or not _G.BearHub_CONFIG_STORE[_G.BearHub_selectedConfigName] then
            cfgStatus("Select a config first!", Color3.fromRGB(255, 100, 100), 2); return
        end
        local data = _G.BearHub_serializeCurrentConfig()
        _G.BearHub_CONFIG_STORE[_G.BearHub_selectedConfigName] = data
        if FS_AVAILABLE then
            local ok = _G.BearHub_saveConfigToFile(_G.BearHub_selectedConfigName, data)
            if ok then
                cfgStatus("Saved '" .. _G.BearHub_selectedConfigName .. "' to disk", Color3.fromRGB(140, 120, 255), 2)
            else
                cfgStatus("Saved '" .. _G.BearHub_selectedConfigName .. "' (memory)", Color3.fromRGB(255, 180, 60), 2)
            end
        else
            cfgStatus("Saved '" .. _G.BearHub_selectedConfigName .. "' (memory)", Color3.fromRGB(255, 180, 60), 2)
        end
    ...
    cfgDeleteBtn.MouseButton1Click:Connect(function()
        ...
        local target = _G.BearHub_selectedConfigName
        _G.BearHub_CONFIG_STORE[target] = nil
        removeConfigFromList(target)
        if FS_AVAILABLE then _G.BearHub_deleteConfigFile(target) end
    ...
    cfgLoadBtn.MouseButton1Click:Connect(function()
        ...
        _G.BearHub_loadConfigData(_G.BearHub_CONFIG_STORE[_G.BearHub_selectedConfigName])
        pcall(refreshAllUI)
        cfgStatus("Loaded '" .. _G.BearHub_selectedConfigName .. "'!", Color3.fromRGB(100, 180, 255), 2)
    end)
    ...
    local pI=Instance.new("TextLabel",panicC); pI.Size=UDim2.new(1,0,0,50); pI.Position=UDim2.new(0,0,0,20); pI.BackgroundTransparency=1; pI.Text="!"; pI.TextColor3=Color3.fromRGB(255,60,60); pI.Font=Enum.Font.GothamBold; pI.TextSize=40
    ...
    local pD=Instance.new("TextLabel",panicC); pD.Size=UDim2.new(1,-30,0,60); pD.Position=UDim2.new(0,15,0,105); pD.BackgroundTransparency=1; pD.TextWrapped=true; pD.Text="Instantly destroys the entire script, removes all traces (ESP, hitbox, GUI, sounds)."; pD.TextColor3=Color3.fromRGB(160,160,170); pD.Font=Enum.Font.Gotham; pD.TextSize=12
    ...
    pBtn.MouseButton1Click:Connect(function() _G.BearHub_PANIC_DESTROY() end)

    local selSS=nil
    local function switchSS(n)
        stGenP.Visible=(n=="General"); stConfigP.Visible=(n=="Config"); stPanicP.Visible=(n=="Panic")
    end
    ...
    local ss1=mkSSB("General",1); mkSSB("Config",2); mkSSB("Panic",3)
...
UIS.InputEnded:Connect(function(inp)
    if inp.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        if not _G.BearHub_PANIC_TRIGGERED then
            _G.BearHub_canvasDrag = false; _G.BearHub_hueDrag = false
            if _G.BearHub_allSliders then
...
print("[BearHub] Part 2 loaded! GUI ready.")
