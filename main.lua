...
do
	...
	local function makeText(parent, sz)
		...
		t.BackgroundTransparency = 1; t.Font = TEXT_FONT; t.TextSize = sz or 14
	...
	local function createESPData(plr)
		...
		local d = {
			...
			distance=makeText(h, 12), invLines={}
		}
		for i = 1, 12 do d.skeleton[i] = makeLine(h) end
		for i = 1, MAX_INV_LINES do
			d.invLines[i] = makeText(h, 11)
		end
							...
							if not skip then
								...
								if not skip then
									...
									if ESP.Inventory.Enabled then
										...
										local lineCount = math.min(#items, MAX_INV_LINES)
										for i = 1, MAX_INV_LINES do
											if i <= lineCount then
												local itemName = items[i]
												if #itemName > 25 then itemName = string.sub(itemName, 1, 22) .. "..." end
												d.invLines[i].Text = itemName
												d.invLines[i].TextColor3 = ESP.Inventory.Color
												d.invLines[i].Position = UDim2.new(0, sp.X, 0, bY + 12 + bo)
												d.invLines[i].Size = UDim2.new(0, 300, 0, 16)
												d.invLines[i].Visible = true
												bo = bo + 14
											elseif lineCount == 0 and i == 1 then
												d.invLines[i].Text = "[Empty]"
												d.invLines[i].TextColor3 = Color3.fromRGB(120, 120, 130)
												d.invLines[i].Position = UDim2.new(0, sp.X, 0, bY + 12 + bo)
												d.invLines[i].Size = UDim2.new(0, 300, 0, 16)
												d.invLines[i].Visible = true
												bo = bo + 14
											else
												d.invLines[i].Visible = false
											end
										end
									else
										for i = 1, MAX_INV_LINES do
			...
			if plr ~= player then
				...
				if char then
					...
					if hum and head and root then
						...
						if dc then
							local ok, sp2, on, d2 = pcall(function()
		...
		for _, plr in ipairs(Players:GetPlayers()) do
			if plr ~= player and plr.Character then
				...
				if hum and hum.Health > 0 and tRoot then
					local dist2 = (myRoot.Position - tRoot.Position).Magnitude
					if dist2 <= bestDist then
						local dir = (tRoot.Position - myRoot.Position).Unit
						if dir:Dot(myRoot.CFrame.LookVector) > 0 then best = plr; bestDist = dist2 end
					end
				end
			end
		end
...
do
	...
	contentTitle.TextColor3 = Color3.new(1,1,1); contentTitle.Font = TEXT_FONT
	...
	cpTitleLbl.Font = TEXT_FONT; cpTitleLbl.TextSize = 14; cpTitleLbl.ZIndex = 101
	...
	cpX.TextColor3 = Color3.fromRGB(200,200,200); cpX.Font = TEXT_FONT
	...
	local cpApply = Instance.new("TextButton", colorPickerGui); cpApply.Size = UDim2.new(0,80,0,28); cpApply.Position = UDim2.new(1,-95,0,223); cpApply.BackgroundColor3 = PURPLE; cpApply.Text = "Apply"; cpApply.TextColor3 = Color3.new(1,1,1); cpApply.Font = TEXT_FONT; cpApply.TextSize = 13; cpApply.ZIndex = 102; cpApply.AutoButtonColor = false
...
do
	...
	mkSection = function(p,t,o)
		...
		l.Text=t; l.TextColor3=Color3.fromRGB(160,160,170); l.Font=TEXT_FONT
	...
	mkCheck = function(p,t,tbl,k,o)
		...
		lb.Font=TEXT_FONT; lb.TextSize=13; lb.TextXAlignment=Enum.TextXAlignment.Left
	...
	mkSlider = function(p,t,minV,maxV,def,suf,tbl,k,o)
		...
		vl.Font=TEXT_FONT; vl.TextSize=13; vl.TextXAlignment=Enum.TextXAlignment.Right
	...
	mkDropdown = function(p,t,opts,tbl,k,o)
		...
		lb.Text=t; lb.TextColor3=Color3.fromRGB(200,200,210); lb.Font=TEXT_FONT
		...
		btn.TextColor3=Color3.fromRGB(200,200,210); btn.Font=TEXT_FONT; btn.TextSize=13
		...
		for _,opt in ipairs(opts) do
			...
			ob.TextColor3=Color3.fromRGB(180,180,190); ob.Font=TEXT_FONT; ob.TextSize=13
	...
	mkKeybind = function(p,t,tbl,o)
		...
		keyBtn.TextColor3=Color3.fromRGB(180,180,190); keyBtn.Font=TEXT_FONT; keyBtn.TextSize=11
		...
		nb.Text="  NONE"; nb.TextColor3=Color3.fromRGB(150,150,160); nb.Font=TEXT_FONT; nb.TextSize=12
		...
		for i,opt in ipairs(BIND_OPTIONS) do
			...
			ob.Text="  "..name; ob.TextColor3=Color3.fromRGB(180,180,190); ob.Font=TEXT_FONT; ob.TextSize=12
	...
	mkButton = function(p,t,cb,o,cc)
		...
		btn.TextColor3=Color3.new(1,1,1); btn.Font=TEXT_FONT; btn.TextSize=13
...
do
	...
	local function mkSB(n,o)
		...
		btn.Text=n; btn.TextColor3=Color3.fromRGB(120,120,130); btn.Font=TEXT_FONT; btn.TextSize=14; btn.AutoButtonColor=false; btn.LayoutOrder=o
	...
	local function startFreeCam()
		...
		task.spawn(function()
			while freecamActive and MISC.FreeCam and not PANIC_TRIGGERED do
				...
				pcall(function()
					Camera.CFrame = CFrame.new(camPos2) * rotCF
				end)
			end
			stopFreeCam()
		end)
	...
	local function mkMSB(n,dn,o)
		...
		btn.Text=dn; btn.TextColor3=Color3.fromRGB(120,120,130); btn.Font=TEXT_FONT; btn.TextSize=12; btn.AutoButtonColor=false; btn.LayoutOrder=o
		local ul=Instance.new("Frame",btn); ul.Size=UDim2.new(1,0,0,2); ul.Position=UDim2.new(0,0,1,-2); ul.BackgroundColor3=PURPLE; ul.BorderSizePixel=0; ul.Visible=false
		btn.MouseButton1Click:Connect(function()
	...
	local plLT=Instance.new("TextLabel",plLF); plLT.Size=UDim2.new(1,-100,0,25); plLT.Position=UDim2.new(0,10,0,5); plLT.BackgroundTransparency=1; plLT.Text="Players in Server"; plLT.TextColor3=Color3.fromRGB(160,160,170); plLT.Font=TEXT_FONT; plLT.TextSize=14; plLT.TextXAlignment=Enum.TextXAlignment.Left
	local plCL=Instance.new("TextLabel",plLF); plCL.Size=UDim2.new(0,85,0,25); plCL.Position=UDim2.new(1,-90,0,5); plCL.BackgroundTransparency=1; plCL.Text="0 players"; plCL.TextColor3=Color3.fromRGB(150,150,160); plCL.Font=TEXT_FONT; plCL.TextSize=12; plCL.TextXAlignment=Enum.TextXAlignment.Right
	...
	local plIT=Instance.new("TextLabel",plIF); plIT.Size=UDim2.new(1,-20,0,22); plIT.Position=UDim2.new(0,10,0,8); plIT.BackgroundTransparency=1; plIT.Text="Selected Player"; plIT.TextColor3=Color3.fromRGB(160,160,170); plIT.Font=TEXT_FONT; plIT.TextSize=14; plIT.TextXAlignment=Enum.TextXAlignment.Left
	...
	local plNL=Instance.new("TextLabel",plIF); plNL.Size=UDim2.new(1,-20,0,18); plNL.Position=UDim2.new(0,10,0,100); plNL.BackgroundTransparency=1; plNL.Text="No player selected"; plNL.TextColor3=Color3.new(1,1,1); plNL.Font=TEXT_FONT; plNL.TextSize=14; plNL.TextXAlignment=Enum.TextXAlignment.Center
	local plUL=Instance.new("TextLabel",plIF); plUL.Size=UDim2.new(1,-20,0,15); plUL.Position=UDim2.new(0,10,0,119); plUL.BackgroundTransparency=1; plUL.Text=""; plUL.TextColor3=Color3.fromRGB(150,150,160); plUL.Font=TEXT_FONT; plUL.TextSize=11; plUL.TextXAlignment=Enum.TextXAlignment.Center
	local plIL=Instance.new("TextLabel",plIF); plIL.Size=UDim2.new(0.5,-15,0,15); plIL.Position=UDim2.new(0,10,0,136); plIL.BackgroundTransparency=1; plIL.Text=""; plIL.TextColor3=Color3.fromRGB(150,150,160); plIL.Font=TEXT_FONT; plIL.TextSize=11; plIL.TextXAlignment=Enum.TextXAlignment.Center
	local plDL=Instance.new("TextLabel",plIF); plDL.Size=UDim2.new(0.5,-15,0,15); plDL.Position=UDim2.new(0.5,5,0,136); plDL.BackgroundTransparency=1; plDL.Text=""; plDL.TextColor3=Color3.fromRGB(100,200,255); plDL.Font=TEXT_FONT; plDL.TextSize=11; plDL.TextXAlignment=Enum.TextXAlignment.Center
	...
	local plSB=Instance.new("TextButton",r1); plSB.Size=UDim2.new(0.5,-3,1,0); plSB.BackgroundColor3=PURPLE; plSB.BorderSizePixel=0; plSB.Text="Spectate"; plSB.TextColor3=Color3.new(1,1,1); plSB.Font=TEXT_FONT; plSB.TextSize=13; plSB.AutoButtonColor=false; Instance.new("UICorner",plSB).CornerRadius=UDim.new(0,6)
	local plUB=Instance.new("TextButton",r1); plUB.Size=UDim2.new(0.5,-3,1,0); plUB.Position=UDim2.new(0.5,3,0,0); plUB.BackgroundColor3=Color3.fromRGB(180,60,60); plUB.BorderSizePixel=0; plUB.Text="Unspectate"; plUB.TextColor3=Color3.new(1,1,1); plUB.Font=TEXT_FONT; plUB.TextSize=13; plUB.AutoButtonColor=false; Instance.new("UICorner",plUB).CornerRadius=UDim.new(0,6)
	...
	local plTB=Instance.new("TextButton",r2); plTB.Size=UDim2.new(0.5,-3,1,0); plTB.BackgroundColor3=Color3.fromRGB(60,140,220); plTB.BorderSizePixel=0; plTB.Text="Teleport"; plTB.TextColor3=Color3.new(1,1,1); plTB.Font=TEXT_FONT; plTB.TextSize=13; plTB.AutoButtonColor=false; Instance.new("UICorner",plTB).CornerRadius=UDim.new(0,6)
	local plBB=Instance.new("TextButton",r2); plBB.Size=UDim2.new(0.5,-3,1,0); plBB.Position=UDim2.new(0.5,3,0,0); plBB.BackgroundColor3=Color3.fromRGB(80,180,100); plBB.BorderSizePixel=0; plBB.Text="Bring"; plBB.TextColor3=Color3.new(1,1,1); plBB.Font=TEXT_FONT; plBB.TextSize=13; plBB.AutoButtonColor=false; Instance.new("UICorner",plBB).CornerRadius=UDim.new(0,6)
	local plSW=Instance.new("TextButton",plIF); plSW.Size=UDim2.new(1,-20,0,28); plSW.Position=UDim2.new(0,10,0,226); plSW.BackgroundColor3=Color3.fromRGB(220,150,50); plSW.BorderSizePixel=0; plSW.Text="Switch Places"; plSW.TextColor3=Color3.new(1,1,1); plSW.Font=TEXT_FONT; plSW.TextSize=13; plSW.AutoButtonColor=false; Instance.new("UICorner",plSW).CornerRadius=UDim.new(0,6)
	local plSL=Instance.new("TextLabel",plIF); plSL.Size=UDim2.new(1,-20,0,20); plSL.Position=UDim2.new(0,10,1,-28); plSL.BackgroundTransparency=1; plSL.Text=""; plSL.TextColor3=Color3.fromRGB(100,200,100); plSL.Font=TEXT_FONT; plSL.TextSize=12; plSL.TextXAlignment=Enum.TextXAlignment.Center
	...
	local function createPB(plr)
		...
		local nl=Instance.new("TextLabel",btn); nl.Size=UDim2.new(1,-45,0,18); nl.Position=UDim2.new(0,42,0,3); nl.BackgroundTransparency=1; nl.Text=plr.DisplayName or plr.Name; nl.TextColor3=Color3.new(1,1,1); nl.Font=TEXT_FONT; nl.TextSize=13; nl.TextXAlignment=Enum.TextXAlignment.Left; nl.TextTruncate=Enum.TextTruncate.AtEnd
		local ul2=Instance.new("TextLabel",btn); ul2.Size=UDim2.new(1,-45,0,16); ul2.Position=UDim2.new(0,42,0,21); ul2.BackgroundTransparency=1; ul2.Text="@"..plr.Name; ul2.TextColor3=Color3.fromRGB(150,150,160); ul2.Font=TEXT_FONT; ul2.TextSize=11; ul2.TextXAlignment=Enum.TextXAlignment.Left; ul2.TextTruncate=Enum.TextTruncate.AtEnd
	...
	local mbH2=Instance.new("Frame",sgPanel); mbH2.Size=UDim2.new(1,0,0,30); mbH2.BackgroundTransparency=1; mbH2.LayoutOrder=2
	local mbLbl=Instance.new("TextLabel",mbH2); mbLbl.Size=UDim2.new(0.5,0,1,0); mbLbl.Position=UDim2.new(0,5,0,0)
	mbLbl.BackgroundTransparency=1; mbLbl.Text="Toggle Menu Key"; mbLbl.TextColor3=Color3.fromRGB(200,200,210)
	mbLbl.Font=TEXT_FONT; mbLbl.TextSize=13; mbLbl.TextXAlignment=Enum.TextXAlignment.Left
	local mbBtn=Instance.new("TextButton",mbH2); mbBtn.Size=UDim2.new(0,120,0,24); mbBtn.Position=UDim2.new(1,-125,0.5,-12)
	mbBtn.BackgroundColor3=Color3.fromRGB(40,40,50); mbBtn.BorderSizePixel=0; mbBtn.Text=MENU_BIND.KeyName
	mbBtn.TextColor3=Color3.fromRGB(180,180,190); mbBtn.Font=TEXT_FONT; mbBtn.TextSize=11
	...
	local pI=Instance.new("TextLabel",panicC); pI.Size=UDim2.new(1,0,0,50); pI.Position=UDim2.new(0,0,0,20); pI.BackgroundTransparency=1; pI.Text="⚠️"; pI.TextColor3=Color3.fromRGB(255,60,60); pI.Font=TEXT_FONT; pI.TextSize=40
	local pT=Instance.new("TextLabel",panicC); pT.Size=UDim2.new(1,-20,0,25); pT.Position=UDim2.new(0,10,0,75); pT.BackgroundTransparency=1; pT.Text="PANIC BUTTON"; pT.TextColor3=Color3.fromRGB(255,80,80); pT.Font=TEXT_FONT; pT.TextSize=20
	local pD=Instance.new("TextLabel",panicC); pD.Size=UDim2.new(1,-30,0,60); pD.Position=UDim2.new(0,15,0,105); pD.BackgroundTransparency=1; pD.TextWrapped=true; pD.Text="Instantly destroys the entire script, removes all traces (ESP, hitbox, GUI, sounds). It will look like you never used any cheats."; pD.TextColor3=Color3.fromRGB(160,160,170); pD.Font=TEXT_FONT; pD.TextSize=12
	local pBtn=Instance.new("TextButton",panicC); pBtn.Size=UDim2.new(0.6,0,0,60); pBtn.Position=UDim2.new(0.2,0,0,185); pBtn.BackgroundColor3=Color3.fromRGB(200,30,30); pBtn.BorderSizePixel=0; pBtn.Text="SPIERDALAJ"; pBtn.TextColor3=Color3.new(1,1,1); pBtn.Font=TEXT_FONT; pBtn.TextSize=24; pBtn.AutoButtonColor=false; Instance.new("UICorner",pBtn).CornerRadius=UDim.new(0,10)
	...
	local function mkSSB(n,o)
		local btn=Instance.new("TextButton",stSubBar); btn.Size=UDim2.new(0,100,1,0); btn.BackgroundTransparency=1; btn.BorderSizePixel=0; btn.Text=n; btn.TextColor3=Color3.fromRGB(120,120,130); btn.Font=TEXT_FONT; btn.TextSize=14; btn.AutoButtonColor=false; btn.LayoutOrder=o
		local ul=Instance.new("Frame",btn); ul.Size=UDim2.new(1,0,0,2); ul.Position=UDim2.new(0,0,1,-2); ul.BackgroundColor3=PURPLE; ul.BorderSizePixel=0; ul.Visible=false
	...
	local afL=Instance.new("TextLabel",afP); afL.Size=UDim2.new(1,-20,0,40); afL.Position=UDim2.new(0,10,0,10); afL.BackgroundTransparency=1; afL.Text="AutoFarm - Coming Soon"; afL.TextColor3=Color3.fromRGB(100,100,110); afL.Font=TEXT_FONT; afL.TextSize=16
end
...
local function makeTabBtn(name, order)
	...
	btn.Font = TEXT_FONT; btn.TextSize = 14
	btn.TextXAlignment = Enum.TextXAlignment.Left; btn.AutoButtonColor = false
